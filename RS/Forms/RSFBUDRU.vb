Public Class RSFBUDRU
    Dim rowARTCUST1 As DataRow
    Dim CUST_CODE As String
    Dim RSTBUDRX As String
    Dim WKS As Integer
    Dim YWs() As String
    Dim RSTBUDRU As String

#Region "ABS Standard Routines" ' These Routines should be found in all Forms which Launch from the Menu.

    Private Sub Form_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load

        If MENU_ITEM_OBJECT = "RSFBUDRU" Then
            RSTBUDRU = "RSTBUDRU"
        Else
            RSTBUDRU = "RSTBUDRS"
        End If
        ...
        With dst
            ...
            Create_TDA(.Tables.Add, RSTBUDRU, "*")
            Create_BAs(RSTBUDRU)
            ...
            With .Tables("RSTBUDRX").Columns
                ...
                For W As Integer = 1 To 53
                    If RSTBUDRU = "RSTBUDRU" Then
                        dst.Tables("RSTBUDRX").Columns("BUDGET_P" & Format(W, "00")).DataType = GetType(System.Int64)
                    End If
        ...
        With grdRSFBUDRX.DisplayLayout.Bands("RSTBUDRX")
            For Each COLUMN_NAME As String In New String() _
                {"ITEM_CODE", "CUST_CODE", "CUST_STORE_NO"}
            ...
            For Each gcol As UltraWinGrid.UltraGridColumn In .Columns
                ...
                If New String() {"CUST_CODE", "CUST_STORE_NO", "ITEM_CODE", "TOTAL", "CUST_STORE_LOCATION", "CUST_STORE_CLASS_CODE", "CUST_STORE_CITY", "CUST_STORE_STATE"}.Contains(gcol.Key) Then
    ...
    Overrides Sub Proceed_PreReq(ByVal eItemKey As String)
        ...
        Select Case eItemKey
            ...
            Case "Load"
                ...
                If EMsg = "" Then
                    If Not ASCMAIN1.Logical_Lock(RSTBUDRU, "*") Then
                        Exit Sub
                    End If
                End If

            Case "Update"
                Dim CUST_CODE_loaded As String = Absx1.txtFor("CUST_CODE").Text
                For Each row As DataRow In ASCDATA1.SelectDistinct(dst.Tables("RSTBUDRX"), New String() {"CUST_CODE", "CUST_STORE_NO"}).Select("")
                    ...
                    If CUST_CODE_loaded <> "" And CUST_CODE <> CUST_CODE_loaded Then
                ...
                For Each row As DataRow In ASCDATA1.SelectDistinct(dst.Tables("RSTBUDRX"), New String() {"ITEM_CODE"}).Select("")
                    Dim ITEM_CODE As String = row.Item(0)
                    Dim rowICTITEM1 As DataRow = LookUp("ICTITEM1", ITEM_CODE)
                    If rowICTITEM1 Is Nothing Then
                        EMsg &= vbCr & "Invalid Item (" & ITEM_CODE & ")"
                    End If
                Next
            ...
            Case "Import from XLS"
                ...
                If MsgBox("This function will Import Retail Sales Budget " & IIf(RSTBUDRU = "RSTBUDRU", "Units", "Sales") & " data, by SKU/Store/Week" _
        ...
        Select Case eItemKey
            ...
            Case "Import from XLS"
                ...
                With grdRSFBUDRX.DisplayLayout.Bands(0)
                    For Each COLUMN_NAME As String In New String() {"ITEM_CODE", "CUST_CODE", "CUST_STORE_NO"}
                ...
                Sort_grdColumns(grdRSFBUDRX, "ITEM_CODE,CUST_CODE,CUST_STORE_NO")
                Setup_grd()

                For Each row As DataRow In dst.Tables("RSTBUDRX").Select("")
                    row.Item("ITEM_CODE") = Trim(row.Item("ITEM_CODE") & "")
                Next
        End Select
    End Sub
    ...
    Overrides Sub Mode_Settings(ByVal tf As Boolean, Optional ByVal MODE_description As String = "")
        ...
        With grdRSFBUDRX.DisplayLayout.Bands(0)
            ...
            .Columns("ITEM_CODE").Hidden = ScreenMode And (EntryMode = "L")
        End With
    ...
    Sub Update_Record()
        ...
        For Each rowRSTBUDRX As DataRow In dst.Tables("RSTBUDRX").Select("")
            For I As Integer = 1 To WKS
                ...
                If BUDGET <> 0 Then
                    Dim row As DataRow = dst.Tables(RSTBUDRU).NewRow
                    row.Item("ITEM_CODE") = rowRSTBUDRX.Item("ITEM_CODE")
                    row.Item("CUST_CODE") = rowRSTBUDRX.Item("CUST_CODE")
                    row.Item("CUST_STORE_NO") = rowRSTBUDRX.Item("CUST_STORE_NO")
                    row.Item("OPS_YYYYWW") = YWs(I)
                    row.Item("BUDGET") = BUDGET
                    dst.Tables(RSTBUDRU).Rows.Add(row)
                End If
            Next
        Next

        ' Update_Record_TDA(RSTBUDRU, sql_Delete)
...
#End Region
    ...
    Sub Setup_grd()
        ...
        grdRSFBUDRX.Text = "Retail Sales Budgets " & IIf(RSTBUDRU = "RSTBUDRU", "Units", "Sales") & ", by SKU/Store/Week, for " & RYP_LEGENDS & " - " & COLLS
     ...
     Optional ByRef F As ASFEXCL1 = Nothing) As Int64
        ...
        For Each row As DataRow In dt.Select("")
            ...
            Try
                ...
                With rowRSTBUDRX
                    For Each C As String In New String() {"ITEM_CODE", "CUST_CODE", "CUST_STORE_NO"}
                        .Item(C) = row.Item(C)
                    Next
                    For I As Integer = 1 To WKS
    ...
    Sub Create_Work_Tables()
        ...
        For I As Integer = 1 To 53
            ...
            If YW > RYW1 Then YW = ""
            sqlM &= ", Sum (Decode(B.OPS_YYYYWW,'" & YW & "',BUDGET,0)) BUDGET_P" & Format(I, "00")
        Next

        ASCMAIN1.sql = "Select B.ITEM_CODE, B.CUST_CODE, B.CUST_STORE_NO" & vbCrLf _
            & sqlM _
            & " from " & RSTBUDRU & " B,ARTCUST2" _
            & " where B.OPS_YYYYWW between '" & RYW0 & "' and '" & RYW1 & "'" & vbCrLf _
            & "   and ARTCUST2.CUST_CODE = B.CUST_CODE and ARTCUST2.CUST_STORE_NO = B.CUST_STORE_NO" & vbCrLf _
            & IIf(CUST_CODE = "", "", "   and B.CUST_CODE = '" & CUST_CODE & "'" & vbCrLf) _
            & " group by B.ITEM_CODE, B.CUST_CODE, B.CUST_STORE_NO"

        If RSTBUDRX = "" Then
            RSTBUDRX = ASCMAIN1.Temp_Table
            ASCDATA1.ExecuteSQL("Alter Table " & RSTBUDRX & " Add Primary Key (ITEM_CODE, CUST_CODE, CUST_STORE_NO)")
        Else
