Public Class RSRGOAL1
    ...
    Sub Create_Pivot()
        ...
        Dim M(6) As String
        M(0) = IIf(SEASON_TYPE = "S", SEASON_YEAR_LY & "12", SEASON_YEAR & "06")
        M(1) = IIf(SEASON_TYPE = "S", SEASON_YEAR & "01", SEASON_YEAR & "07")
        M(2) = IIf(SEASON_TYPE = "S", SEASON_YEAR & "02", SEASON_YEAR & "08")
        M(3) = IIf(SEASON_TYPE = "S", SEASON_YEAR & "03", SEASON_YEAR & "09")
        M(4) = IIf(SEASON_TYPE = "S", SEASON_YEAR & "04", SEASON_YEAR & "10")
        M(5) = IIf(SEASON_TYPE = "S", SEASON_YEAR & "05", SEASON_YEAR & "11")
        M(6) = IIf(SEASON_TYPE = "S", SEASON_YEAR & "06", SEASON_YEAR & "12")
        ...
        For Each row As DataRow In ASCDATA1.GetDataTable.Select("", "YYYYWW")
            W += 1
            Dim WC As String = "ST_P" & Format(W, "00")
            Dim YW As String = row.Item("YYYYWW")
            If Mid(YW, 5, 2) = "53" Then
            ElseIf Mid(YW, 5, 2) = "52" Then
                SQLP &= ", Sum (Decode(RSTRETL1.OPS_YYYYWW,'" & YW & "',RSTRETL1.QTY_SOLD,'" & Mid(YW, 1, 4) & "53" & "',RSTRETL1.AMT_SOLD,0)) " & WC & vbCrLf
            Else
                SQLP &= ", Sum (Decode(RSTRETL1.OPS_YYYYWW,'" & YW & "',RSTRETL1.QTY_SOLD,0)) " & WC & vbCrLf
            End If
            SQLP0 &= ", 0 " & WC
        Next
        SQLP &= vbCrLf
        ...
        For I As Integer = 1 To 6
            ...
            For Each row As DataRow In ASCDATA1.GetDataTable.Select("", "YYYYWW")
                W += 1
                Dim WC As String = "ST_W" & Format(W, "00")
                Dim YW As String = row.Item("YYYYWW")
                SQLM &= ", Sum (Decode(RSTRETL1.OPS_YYYYWW,'" & YW & "',RSTRETL1.QTY_SOLD,0)) " & WC & vbCrLf
                SQLM0 &= ", 0 " & WC
            Next
            SQLM &= vbCrLf
            SQLM0 &= vbCrLf
        ...
        ASCMAIN1.sql = "Select X.* from " & IIf(chkALLDOORS.Checked, "", "SATAUTH1, ") & "ICTBRAN1, ICTCOLL0, (" & vbCrLf _
            ...
            & ", Sum (ST_W22) ST_W22, Sum (ST_W23) ST_W23, Sum (ST_W24) ST_W24, Sum (ST_W25) ST_W25, Sum (ST_W26) ST_W26" & vbCrLf _
            & " from (" & vbCrLf _
            ...
            & ", SUM (CASE WHEN TO_CHAR(SOTORDR1.ORDR_SHIP_DATE,'YYYYMM') BETWEEN '" & M(1) & "' AND '" & M(6) & "' THEN SOTORDR2.ORDR_QTY_OPEN ELSE 0 END) QTY_OPEN" & vbCrLf _
            & ", 0 SI_TOTAL, 0 SI_M00, 0 SI_M01, 0 SI_M02, 0 SI_M03, 0 SI_M04, 0 SI_M05, 0 SI_M06" & vbCrLf _
            & ", 0 ST_TOTAL" & SQLP0 & SQLM0 _
            & " from SOTORDR1, SOTORDR2, ICTITEM1, ICTCOLL1, ARTCUST1" & vbCrLf _
        ...
        ws = wb.Worksheets("F14")

        ' WEEK 53 - JUST LUMP IN WITH P05?
        ...
        ASCMAIN1.sql = "Select " & vbCrLf _
            ...
            & ", ICTCOLL1.COLLECTION_NAME DoorID" & vbCrLf _
            ...
            & ", X.ST_W01, X.ST_W02, X.ST_W03, X.ST_W04" & vbCrLf _
            ...
            & ", X.ST_W22, X.ST_W23, X.ST_W24, X.ST_W25, X.ST_W26" & vbCrLf _
            & " from " & RSTSSTW1 & " X" & vbCrLf _
            & ",ICTBRAN1,ARTCUST1,ARTCUST2,SOTSELL1,SOTSREG1,ICTCOLL0,ICTCOLL1,ICTITEM1" & vbCrLf _
        ...
        For Each row As DataRow In DataTable.Select("", "ChainName,BRAND,BrandName") ' "CUST_NAME,HC_NAME,CUST_STORE_NO")
            r += 1
            ws.Range("E" & CStr(r0 + r) & ":BK" & CStr(r0 + r)).Value2 = row.ItemArray

            If r Mod 100 = 0 Then
                ASCMAIN1.Progress("-", CStr(r) & "/" & CStr(rTotal))
            End If
        Next
        wb.Names.Add("PivotBase", "=MAINDATA!$A$" & CStr(r0) & ":$BK$" & CStr(r0 + DataTable.Rows.Count))
