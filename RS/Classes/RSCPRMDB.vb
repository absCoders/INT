...
#Region "Class Variables"
    ...
    Private clsOracleTableNamePrefix As String
    Private clsOracleSchema As String
    Private clsMdbTable As DataTable = Nothing

    Private clsOracleLeadingFields As String
    Private clsOracleTrailingFields As String
    Private clsOraclePrimaryKey As String
...
#Region "Class Properties / Property type Procedures"
    ...
    ''' <summary>
    ''' Get / Set Oracle Schema Load Create and Load Data into
    ''' </summary>
    ''' <value></value>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Property OracleSchema() As String
        Get
            Return clsOracleSchema
        End Get
        Set(ByVal value As String)
            clsOracleSchema = value.Trim
        End Set
    End Property
    ...
    ''' <summary>
    ''' Get / Set Additional fields appended to front of the oracle table definition
    ''' </summary>
    ''' <value></value>
    ''' <returns></returns>
    ''' <remarks>Comma Delimited fields as they apppear in a oracle Table definition. No leading or trailing commas.</remarks>
    Public Property OracleLeadingFields() As String
        Get
            Return clsOracleLeadingFields
        End Get
        Set(ByVal value As String)
            clsOracleLeadingFields = value.Trim
        End Set
    End Property

    ''' <summary>
    ''' Get / Set Additional fields appended to tail of the oracle table definition
    ''' </summary>
    ''' <value></value>
    ''' <returns></returns>
    ''' <remarks>Comma Delimited fields as they apppear in a oracle Table definition. No leading or trailing commas.</remarks>
    Public Property OracleTrailingFields() As String
        Get
            Return clsOracleTrailingFields
        End Get
        Set(ByVal value As String)
            clsOracleTrailingFields = value.Trim
        End Set
    End Property

    ''' <summary>
    ''' Get / Set Primary key of the Oracle Table
    ''' </summary>
    ''' <value></value>
    ''' <returns></returns>
    ''' <remarks>Comma delimited field list. No other puncuation.</remarks>
    Public Property OraclePrimaryKey() As String
        Get
            Return clsOraclePrimaryKey
        End Get
        Set(ByVal value As String)
            clsOraclePrimaryKey = value.Trim
        End Set
    End Property

    ''' <summary>
    ''' Get / Set a prefis used for the newly Created Oracle Tables
    ''' </summary>
    ''' <value></value>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Property OracleTableNamePrefix() As String
        Get
            Return clsOracleTableNamePrefix
        End Get
        Set(ByVal value As String)
            clsOracleTableNamePrefix = value.Trim
        End Set
    End Property
...
#Region "Public Procedures"
    ...
    Public Function CopyMdbIntoOracle(ByVal vMdbTableName As String, _
        ...
        Try
            ...
            ' If the table does nor exist then create it
            If clsOracleTable Is Nothing Then
                Me.CreateOracleTableFromMdbSchema(vMdbTableName, vOracleTableName)
                ErrorList.Add("Could not access Oracle Table: " & vOracleTableName)
                Return Nothing
            End If

            sql = "Select * From " & vOracleTableName & " Where Rownum < 1"
            clsOracleTable = ASCDATA1.GetDataTable(sql, String.Empty, 0, False, 0)
            If clsOracleTable Is Nothing Then
                ErrorList.Add("Could not access Oracle Table: " & vOracleTableName)
                Return Nothing
            ...
            Dim FF() As Int32 = Nothing
            Dim FT() As Int32 = Nothing

            Dim totalRows As Long = clsMdbTable.Rows.Count
            For Each mdbRow As DataRow In clsMdbTable.Rows
                ...
                If recCount = 0 Then
                    For i As Integer = 0 To clsOracleTable.Columns.Count - 1
                        ...
                        If mdbRow.Table.Columns.Contains(fieldname) Then
                            Dim f As Int32 = -1
                            If FF IsNot Nothing Then
                                f = UBound(FF)
                            End If
                            ReDim Preserve FF(f + 1)
                            ReDim Preserve FT(f + 1)
                            FT(f + 1) = i
                            FF(f + 1) = mdbRow.Table.Columns(fieldname).Ordinal
                        End If
                    Next
                End If

                recCount += 1
                If displayControl IsNot Nothing And recCount Mod 50 = 0 Then
                    Try
                        displayControl.Text = Format((recCount / totalRows) * 100, "0.00") & "%"
                ...
                Else
                    For i As Int32 = 0 To UBound(FF)
                        clsDataRow.Item(FT(i)) = mdbRow.Item(FF(i))
                    Next
                    'Stop
                    'For i As Integer = 0 To clsOracleTable.Columns.Count - 1
                    '    fieldname = clsOracleTable.Columns(i).ColumnName
    ...
    Public Function CreateOracleTableFromMdbSchema(ByVal vMdbTableName As String, ByVal vOracleTableName As String) As DataTable
        ...
        If Me.clsOracleTableNamePrefix.Trim.Length > 0 And vOracleTableName.Trim.Length > 0 Then
            vOracleTableName = Me.clsOracleTableNamePrefix & vOracleTableName
        End If

        If Me.clsOracleSchema.Trim.Length > 0 And vOracleTableName.Trim.Length > 0 Then
            vOracleTableName = Me.clsOracleSchema & "." & vOracleTableName
        End If
    ...
    Public Function GetOracleDDL(ByVal vMdbTableName As String, ByVal vOracleTableName As String) As String
        ...
        Try
            ...
            oracleDDL = Mid(oracleDDL, 3).Trim

            If Me.clsOracleLeadingFields.Trim.Length > 0 Then
                oracleDDL = Me.clsOracleLeadingFields & ", " & oracleDDL
            End If

            If Me.clsOracleTrailingFields.Trim.Length > 0 Then
                oracleDDL = oracleDDL & ", " & Me.clsOracleLeadingFields
            End If

            If Me.clsOraclePrimaryKey.Trim.Length > 0 Then
                oracleDDL = oracleDDL & ", PRIMARY KEY (" & Me.clsOraclePrimaryKey & ")"
            End If

            oracleDDL = "Create Table " & vOracleTableName & " ( " & oracleDDL & " )"
