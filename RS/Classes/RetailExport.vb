...
#Region "Styling"
    Protected Sub SetStyleAlignCenter(ByRef rng As IRange)
        rng.Style.HorizontalAlignment = HAlign.Center
    End Sub

 

    Protected Sub SetStyleAlignCenterVertical(ByRef rng As IRange)
        rng.Style.VerticalAlignment = VAlign.Center
    End Sub


    Protected Sub SetStyleBackgroundColor(ByRef rng As IRange, ByVal color As Color)
        rng.Interior.Pattern = Pattern.Solid
        rng.Interior.Color = color
    End Sub

    Protected Sub SetStyleFontBold(ByRef rng As IRange)
        rng.Style.Font.Bold = True
    End Sub

    Protected Sub SetStyleFontColor(ByRef rng As IRange, ByVal color As Color)
        rng.Style.Font.Color = color
    End Sub

    Protected Sub SetStyleWrapText(ByRef rng As IRange)
        rng.Style.WrapText = True
    End Sub

    Protected Sub SetNumberFormat(ByRef rng As IRange, ByVal formatString As String)
        rng.Style.NumberFormat = formatString
    End Sub

    Protected Sub SetThousandsFormat(ByVal rng As IRange)
        SetNumberFormat(rng, "$#,##0.0,")
    End Sub

    Protected Sub SetDollarFormat(ByVal rng As IRange)
        SetNumberFormat(rng, "$#,##0")
    End Sub

    Protected Sub SetPercentageFormat(ByVal rng As IRange)
        SetNumberFormat(rng, "#0.#%")
    End Sub
...
Public Class WeeklyFlashByAccount
    ...
    Public Sub CreateReport(ByVal fileName As String, Optional ByVal showWTDbyStore As Boolean = False)
        ...
        Dim reportWorkbook As IWorkbook = workbookSet.Workbooks.Add()
        ...
        For Each department As String In departments
            Dim reportSheet As IWorksheet = reportWorkbook.Worksheets.Add()
            reportSheet.Name = department
            ...
            reportSheet.Cells("A").AutoFit()
        Next

        Try
            reportWorkbook.SaveAs(fileName, FileFormat.OpenXMLWorkbook)
    ...
    Private Sub WriteDepartmentSheet(ByVal departmentCode As String, ByVal reportSheet As IWorksheet)
...
BypassDirect:
        ...
        reportSheet.HPageBreaks.Add(reportSheet.Cells(currentRow + 1))
        ...
        If _reportData.AsEnumerable().Any(Function(row) (row.Item("DEPT_CODE") = departmentCode Or departmentCode = "B") And New String() {"G", "C", "T"}.Contains(row.Item("MATL_CODE"))) Then 'CONSIGNMENT DATA EXISTS
            ...
            'reportSheet.Cells(currentRow, 0, 1, 13).SetOutlineBorder(BorderType.TopBorder, CellBorderType.Medium, Colors.Black)
            reportSheet.Cells(currentRow, 0).Value = "CONSIGNMENT RECAP - THESE NUMBERS ARE INCLUDED IN TOTAL SALES RECAP ABOVE"
            SetStyleFontColor(reportSheet.Cells(currentRow, 0), Colors.Red)
    ...
    Private Function WriteTotalWTD(ByVal departmentCode As String, ByVal currentRow As Integer, ByVal reportSheet As IWorksheet) As Integer
    ...
    Private Function WriteDirect(ByVal departmentCode As String, ByVal materialsToReport As String(), ByVal currentRow As Object, ByVal summaryRows As List(Of Integer), ByVal reportSheet As IWorksheet) As Integer
        currentRow = WriteDataRows(departmentCode, "DIRECT", materialsToReport, 1, True, currentRow, reportSheet)
        Dim cellsToStyle As IRange = reportSheet.Cells("A" & (currentRow - 1) & ":M" & (currentRow - 1))
        SetTotalsStyle(cellsToStyle)
        Return currentRow
    End Function

    Private Sub WriteReportHeader(ByVal departmentName As String, ByVal reportSheet As IWorksheet)
        ...
        Dim cellsToStyle As IRange = reportSheet.Cells("C1:M3")
        
        cellsToStyle.Interior.Pattern = Pattern.Solid
        cellsToStyle.Interior.Color = Color.FromArgb(255, 255, 255, 153)
        cellsToStyle.Borders(BordersIndex.InsideHorizontal).LineStyle = LineStyle.None

        reportSheet.Cells(0).ColumnWidth = 20.29
        ...
        'rightBorderStyle.SetBorder(BorderType.RightBorder, CellBorderType.Medium, Colors.Black)
    ...
    Private Sub SetHeaderFont(ByVal cellString As String, ByVal reportSheet As IWorksheet)
    ...
    Private Function WriteColumnHeadings(ByVal includePlan As Boolean, ByVal rowIndex As Integer, ByVal reportsheet As IWorksheet) As Integer

        Dim wtdAdjust As Integer = If(_showWTD, 2, 0)

        If includePlan Then
            reportsheet.Cells(rowIndex, 2, 1, 4 + wtdAdjust).Merge()
            reportsheet.Cells(rowIndex, 8 + wtdAdjust, 1, 4).Merge()
        Else
            reportsheet.Cells(rowIndex, 2, 1, 3 + wtdAdjust).Merge()
            reportsheet.Cells(rowIndex, 8 + wtdAdjust, 1, 3).Merge()
        End If
        ...
        If includePlan Then
            ...
            Dim headerRng As IRange = reportsheet.Cells(rowIndex + 1, 0, 1, 7 + wtdAdjust)
            'headerRng.SetOutlineBorder(BorderType.BottomBorder, CellBorderType.Double, Colors.Black)
        Else
            Dim headerRng As IRange = reportsheet.Cells(rowIndex + 1, 0, 1, 5 + wtdAdjust)
            'headerRng.SetOutlineBorder(BorderType.BottomBorder, CellBorderType.Double, Colors.Black)
        End If
        ...
        If includePlan Then
            ...
            Dim headerRng As IRange = reportsheet.Cells(rowIndex + 1, 8 + wtdAdjust, 1, 5)
            'headerRng.SetOutlineBorder(BorderType.BottomBorder, CellBorderType.Double, Colors.Black)
        Else
            Dim headerRng As IRange = reportsheet.Cells(rowIndex + 1, 8 + wtdAdjust, 1, 3)
            'headerRng.SetOutlineBorder(BorderType.BottomBorder, CellBorderType.Double, Colors.Black)
        End If
    ...
    Private Function WriteDataRows(ByVal departmentCode As String, ByVal rgn As String, ByVal materials As String(), ByVal direct As Integer, ByVal includePlan As Boolean, ByVal currentRow As Integer, ByVal reportSheet As IWorksheet) As Integer
    ...
    Private Sub SetTotalsStyle(ByVal totalsRange As IRange)
        totalsRange.Font.Bold = True
        totalsRange.Interior.Pattern = Pattern.Solid
        totalsRange.Interior.Color = Colors.Yellow
        Dim border As IBorder = totalsRange.Borders(BordersIndex.InsideHorizontal)
        border.Color = Colors.Black
        border.LineStyle = LineStyle.Continuous
        border.Weight = BorderWeight.Thin
    End Sub

    Private Function WriteSubTotalRow(ByVal summaryText As String, ByVal rgnStartRow As Integer, ByVal includePlan As Boolean, ByVal currentRow As Integer, ByVal reportSheet As IWorksheet) As Integer
        Dim rgnEndRow As Integer = currentRow - 1

        Dim cellsToStyle As IRange = reportSheet.Cells(nameCol & currentRow, pctSTDplanCol & currentRow)
        SetTotalsStyle(cellsToStyle)
    ...
    Private Function WriteTotals(ByVal totalText As String, ByVal currentRow As Integer, ByVal summaryRows As List(Of Integer), ByVal reportSheet As IWorksheet) As Integer
...
Public Class NationalWeeklySalesFlash
    ...
    Public Sub CreateReport(ByVal fileName As String)
        ...
        reportSheet.PageSetup.FitToPagesWide = 1
        reportSheet.PageSetup.Orientation = PageOrientation.Landscape
        reportSheet.PageSetup.FitToPagesTall = Nothing

        PrepareSheet(reportSheet)
        FillSheet(reportSheet)

        'reportSheet.Cells("A:A").AutoFit()
        Try
            reportWorkbook.SaveAs(fileName, FileFormat.OpenXMLWorkbook)
        Catch ex As Exception
            MsgBox(ex.Message)
    ...
    Private Sub FillSheet(ByVal reportSheet As IWorksheet)
        ...
        rowIndex = WriteSummarySection("Stores", rowIndex + 3, "CUST_CODE", "AND DIRECT=0", reportSheet, totalRows)
        rowIndex = WriteSummarySection("Direct", rowIndex + 3, "CUST_CODE", "AND DIRECT=1", reportSheet, totalRows)
        rowIndex += 2
    ...
    Private Sub HideWeek5IfNecessary(reportSheet As IWorksheet)
        ...
        If maxWeek < 5 Then
            reportSheet.Cells(0, 13, 0, 15).EntireColumn.Hidden = True
        End If
    End Sub

    Private Function WriteSummarySection(ByVal summaryType As String, ByVal rowIndex As Integer, ByVal groupByColumn As String, ByVal filter As String, reportSheet As IWorksheet, ByRef totalRows As Dictionary(Of String, List(Of Integer))) As Integer
        ...
        If groupByColumn = "CUST_CODE" Then
        ...
        For Each groupByItem As String In groupByItems
            ...
            For Each row As DataRow In _reportData.Select(String.Format("{0}='{1}' " & filter, groupByColumn, groupByItem), "DIRECT DESC")
    ...
    Private Sub WriteDataRow(ByVal dataRow As DataRow, ByVal rowIndex As Integer, ByVal reportSheet As IWorksheet)
        reportSheet.Cells(rowIndex, 0).Value = dataRow.Item("CUST_NAME") & If(Val(dataRow.Item("DIRECT") & "") = 1, " Direct", "")
    ...
    Private Sub WriteSubTotalRow(ByVal summaryType As String, ByVal description As String, ByVal startIndex As Integer, ByVal rowIndex As Integer, ByVal reportSheet As IWorksheet)
        ...
        'reportSheet.Cells(rowIndex, 0, 1, 19).ApplyStyle(subTotalStyle, subTotalFlags)
    ...
    Private Sub WriteSummaryTotals(ByVal summaryType As String, ByVal rowIndex As Integer, ByVal subTotalsRows As List(Of Integer), ByVal reportSheet As IWorksheet)
        'Dim subTotalStyle As Style = reportSheet.Workbook.Styles(reportSheet.Workbook.Styles.Add())
        'subTotalStyle.Font.IsBold = True
        'subTotalStyle.Pattern = BackgroundType.Solid
    ...
    Private Sub WriteTotals(ByVal rowIndex As Integer, ByVal totalRows As Dictionary(Of String, List(Of Integer)), ByVal reportSheet As IWorksheet)
    ...
    Private Sub SetEcommerceStyle(ByVal rowIndex As Integer, ByVal reportSheet As IWorksheet)
        'Dim totalsStyle As Style = reportSheet.Workbook.Styles(reportSheet.Workbook.Styles.Add())
        'totalsStyle.Font.IsBold = True
        'totalsStyle.Pattern = BackgroundType.Solid
    ...
    Private Sub WriteWeekHeaders(ByVal rowIndex As Integer, ByVal reportSheet As IWorksheet)
    ...
    Private Sub WriteColumnHeaders(ByVal rowIndex As Integer, ByVal reportSheet As IWorksheet)
    ...
    Private Sub SetRowBorders(ByVal boldBottom As Boolean, ByVal rowIndex As Integer, reportSheet As IWorksheet)
        'Dim borderFlag As New StyleFlag()
        'borderFlag.BottomBorder = True
    ...
    Private Sub PrepareSheet(ByVal reportSheet As IWorksheet)
...
Public Class MonthlyNationalAndIndependentRecap
    ...
    Public Sub CreateReport(ByVal fileName As String)
        ...
        reportSheet.PageSetup.Orientation = PageOrientation.Landscape
        reportSheet.PageSetup.PaperSize = PaperSize.Letter
    ...
    Private Sub PrepareSheet(ByVal reportSheet As IWorksheet)
        ...
        reportSheet.Cells("A:A").ColumnWidth = 25


        reportSheet.PageSetup.Zoom = 80
        ...
        reportSheet.PageSetup.CenterHeader = String.Format("John Hardy" & vbCrLf &
                                                        "Retail Recap FY20{0}" & vbCrLf &
                                                        "{1} 20{2}", _dateData.Item("FYY"), _dateData.Item("MM"), dateRange)
        reportSheet.PageSetup.LeftFooter = Today.ToString("MM/dd/yyyy")
        reportSheet.PageSetup.CenterFooter = String.Format("John Hardy {1} 20{0}", _dateData.Item("FYY"), _dateData.Item("MM"))
    End Sub
    ...
    Private Sub FillSheet(ByVal reportSheet As IWorksheet)
        ...
        reportSheet.Cells(0, 17).EntireColumn.Hidden = True
        reportSheet.Cells(0, 19).EntireColumn.Hidden = True
        reportSheet.Cells(0, 11).EntireColumn.Delete()
    End Sub

    Private Function WriteLastCallorSaksOff5th(ByVal custName As String, ByVal rowIndex As Integer, ByVal reportSheet As IWorksheet)
        Dim totalRng As IRange = reportSheet.Cells(rowIndex, 0, 1, 20)
        'SetStyleForegroundColor(totalRng, Color.FromArgb(255, 218, 238, 243))
        SetStyleFontBold(totalRng)
        'totalRng.SetOutlineBorder(BorderType.TopBorder, CellBorderType.Medium, Colors.Black)
        'totalRng.SetOutlineBorder(BorderType.BottomBorder, CellBorderType.Medium, Colors.Black)
    ...
    Private Function WriteEcommerce(ByVal rowIndex As Integer, ByVal reportSheet As IWorksheet) As Integer
        Dim totalRng As IRange = reportSheet.Cells(rowIndex, 0, 1, 20)
    ...
    Private Function WriteNationalGrouping(ByVal rowIndex As Integer, ByVal totalText As String, ByVal filter As String, ByVal reportSheet As IWorksheet, ByVal groupDepartment As Boolean) As Integer
    ...
    Private Function WriteIndependents(ByVal rowIndex As Integer, ByVal reportSheet As IWorksheet) As Integer
        Dim startIndex As Integer = rowIndex
    ...
    Private Sub WriteDataRow(ByVal dataRow As DataRow, ByVal writeDepartment As Boolean, ByVal rowIndex As Integer, ByVal reportSheet As IWorksheet)
    ...
    Private Sub SetRowBorders(ByVal totalRow As Boolean, ByVal rowIndex As Integer, reportSheet As IWorksheet)
    ...
    Private Function WriteHeaderRow(ByVal rowIndex As Integer, ByVal reportSheet As IWorksheet) As Integer
        Dim accountRng As IRange = reportSheet.Cells(rowIndex, 0, 2, 1)
        accountRng.Merge()
        ...
        Dim doorRng As IRange = reportSheet.Cells(rowIndex, 1, 2, 1)
        doorRng.Merge()
        ...
        Dim offset2 As Integer = 0
        For i As Integer = 0 To 4
            Dim TyLyRng As IRange = reportSheet.Cells(rowIndex, 2 + (i * 3) + offset, 2, 3)
            If i = 4 Then
                TyLyRng = reportSheet.Cells(rowIndex, 1 + (i * 3) + offset, 2, 5)
            End If
            ...
            Dim headerTextRng As IRange = reportSheet.Cells(rowIndex, 2 + (i * 3) + offset, 1, 3)
            headerTextRng.Merge()
            ...
            If i = 2 Then
                offset += 1
                Dim hdrRange As IRange = reportSheet.Cells(rowIndex, 4 + (i * 3) + offset, 2, 1)
            ...
            If i = 4 Then
                Dim hdrRange As IRange = reportSheet.Cells(rowIndex, 1 + (i * 3) + offset, 2, 4)
                'hdrRange.SetOutlineBorders(CellBorderType.Medium, Color.Black)
            End If
        Next

        reportSheet.Cells("A" & (rowIndex + 1)).EntireRow.AutoFit()

        Return rowIndex + 1
    End Function

    Private Sub WriteSubTotalRow(ByVal startIndex As Integer, ByVal rowIndex As Integer, ByVal totalText As String, ByVal reportSheet As IWorksheet)

        Dim subTotalRange As IRange = reportSheet.Cells(rowIndex, 0, 1, 20)
    ...
    Private Function WriteTotals(ByVal rowIndex As Integer, ByVal totalText As String, ByVal totalRows As List(Of Integer), ByVal reportSheet As IWorksheet, ByVal fgColor As Color) As Integer
        Dim totalRng As IRange = reportSheet.Cells(rowIndex, 0, 1, 20)
...
Public Class MonthlyByDoorRecap
    ...
    Public Sub CreateReport(ByVal fileName As String)
        ...
        For Each deptCode In New String() {"WM", "MN"}
            Dim reportSheet As IWorksheet = reportWorkbook.Worksheets.Add()
            reportSheet.Name = If(deptCode = "WM", "Women", "Men")
    ...
    Private Sub PrepareSheet(ByVal reportSheet As IWorksheet, ByVal deptCode As String)
        'Dim numberFormatFlag As New StyleFlag()
        'numberFormatFlag.NumberFormat = True
    ...
    Private Sub FillSheet(ByVal deptCode As String, ByVal reportSheet As IWorksheet)
        Dim rowIndex As Integer = 0
        rowIndex = WriteHeaderRow(rowIndex, reportSheet)
    ...
    Private Function WriteRegionGrouping(ByVal rowIndex As Integer, ByVal deptCode As String, ByVal regionCode As String, ByVal reportSheet As IWorksheet) As Integer
    ...
    Private Sub WriteRegionTotalRow(ByVal startIndex As Integer, ByVal rowIndex As Integer, ByVal regionDesc As String, ByVal reportSheet As IWorksheet)

        Dim subTotalRange As IRange = reportSheet.Cells(rowIndex, 0, 1, 16)
        SetStyleFontBold(subTotalRange)
        'SetStyleForegroundColor(subTotalRange, Color.FromArgb(255, 255, 255, 204))
    ...
    Private Sub SetRowBorders(ByVal totalRow As Boolean, ByVal rowIndex As Integer, reportSheet As IWorksheet)
        ...
        '    Dim cellStyle = reportSheet.Cells(rowIndex, i).GetStyle()
        '    If i = 0 Then
        '        cellStyle.SetBorder(BorderType.LeftBorder, CellBorderType.Thin, Colors.Black)
        '    End If

        '    If Not totalRow Then
        '        cellStyle.SetBorder(BorderType.BottomBorder, CellBorderType.Thin, Colors.Black)
        '    End If
        '    If mediumBorderCols.Contains(i) Or totalRow Then
        '        cellStyle.SetBorder(BorderType.RightBorder, CellBorderType.Medium, Colors.Black)
        '    Else
        '        cellStyle.SetBorder(BorderType.RightBorder, CellBorderType.Thin, Colors.Black)
        '    End If
    ...
    Private Function WriteHeaderRow(ByVal rowIndex As Integer, ByVal reportSheet As IWorksheet) As Integer
        ...
        For i As Integer = 0 To 4
            'If i = 3 Then offset = 2
            Dim TyLyRng As IRange = reportSheet.Cells(rowIndex, 1 + (i * 3) + offset, 2, 3)
            'SetStyleForegroundColor(TyLyRng, Color.FromArgb(255, 204, 255, 204))
            SetStyleFontBold(TyLyRng)
            SetStyleAlignCenter(TyLyRng)
            'TyLyRng.SetOutlineBorders(CellBorderType.Medium, Colors.Black)

            Dim headerTextRng As IRange = reportSheet.Cells(rowIndex, 1 + (i * 3) + offset, 1, 3)
            headerTextRng.Merge()
            headerTextRng.Value = colHeaders(i)
    ...
    Private Function WriteTotals(ByVal rowIndex As Integer, ByVal totalText As String, ByVal totalRows As List(Of Integer), ByVal reportSheet As IWorksheet) As Integer
        Dim totalRng As IRange = reportSheet.Cells(rowIndex, 0, 1, 16)
...
Public Class IndependentsByRegionReport
    ...
    Public Sub CreateReport(ByVal filename As String)
        ...
        If _regionCode Is Nothing Then 'Create summary sheet
            Dim reportSheet As IWorksheet = reportWorkbook.Worksheets.Add()
            reportSheet.Name = "FISCAL " & _fiscalYear
    ...
    Private Sub FillSheet(ByVal regionCode As String, ByVal reportSheet As IWorksheet)
    ...
    Private Function WriteMonthHeaders(ByVal rowIndex As Integer, ByVal reportSheet As IWorksheet) As Integer
        Dim offset As Integer = 0
        ...
        For i As Integer = 1 To 12 'Each month
            If i = 6 Then
                yyyy += 1
            End If
            Dim rng = reportSheet.Cells(rowIndex, 3 * i + offset, 1, 3)
            rng.Merge()
            ...
            If i Mod 6 = 0 Then
                ...
                Dim stdRng = reportSheet.Cells(rowIndex, 3 * i + offset, 1, 3)
                stdRng.Merge()
                ...
                If i = 12 Then
                    offset = 9
                    Dim fytdRng = reportSheet.Cells(rowIndex, 3 * i + offset, 1, 3)
                    fytdRng.Merge()
    ...
    Private Function WriteYearHeaders(ByVal regionDesc As String, ByVal rowIndex As Integer, ByVal reportSheet As IWorksheet) As Integer
        Dim offset As Integer = 0
        Dim yyyy As Integer = Convert.ToInt32(_fiscalYear) - 1
    ...
    Private Sub SetRowBorders(ByVal boldHorizontals As Boolean, ByVal rowIndex As Integer, ByVal reportsheet As IWorksheet)
    ...
    Private Function WriteSalesRepSection(ByVal srepCode As String, ByVal rowIndex As Integer, ByVal reportSheet As IWorksheet) As Integer
    ...
    Private Function WriteSalesRepTotalRow(ByVal srepName As String, ByVal sumList As String, ByVal rowIndex As Integer, ByVal reportSheet As IWorksheet) As Integer
    ...
    Private Sub WriteSalesRow(ByVal salesRow As DataRow, ByVal rowIndex As Integer, ByVal reportSheet As IWorksheet)
        Dim offset As Integer = 0
        Dim yyyy As Integer = Convert.ToInt32(_fiscalYear) - 1
        ...
        If salesRow.Item("GRP") = "1" Then
            SetStyleFontBold(reportSheet.Cells(rowIndex, 0, 1, 47))
    ...
    Private Sub FormatRow(ByVal rowIndex As Integer, ByVal reportSheet As IWorksheet)
    ...
    Private Function WriteTotals(ByVal regionDesc As String, ByVal sumList As String, ByVal rowIndex As Integer, ByVal reportSheet As IWorksheet) As Integer
