...
Public Class ASCBASE1
    ...
    Sub Print_Report_End( _
    Optional ByVal print_without_showing As Boolean = False, _
    Optional ByVal close_report_viewer As Boolean = False, _
        ...
        For c As Integer = 0 To tbl.Columns.Count - 1
            ...
            Select Case tbl.Columns(c).DataType.ToString
                ...
                Case "System.DateTime"
                    ...
                    ReDim A(row_count - 1)
                    For r As Integer = 0 To row_count - 1
                        If Not tbl.Rows(r).Item(c).Equals(System.DBNull.Value) Then
                            A(r) = tbl.Rows(r).Item(c)
                        End If
                    Next
                    BA_CMDs(TABLE_NAME)(1).Parameters(COLUMN_NAME).Value = A
        ...
        For Each tbl As DataTable In dst.Tables
            For Each row As DataRow In tbl.GetErrors()
                If ASCMAIN1.CLIENT = "SLP" Then
                    Dim rowKEY As String = row.Item(0) & ""
                    ' PROBABLY SHOULD REPRESENT THE ENTIRE KEY IF MULTI-COLUMN
                    ' EVENTUALLY ENABLE THIS SECTION FOR ALL CLIENTS
                    ErrorList.Add("Table: " & tbl.TableName & vbCrLf & rowKEY & ": Row Error: " & row.RowError)
                Else
