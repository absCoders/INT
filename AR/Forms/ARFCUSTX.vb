        ...
        Select Case eItemKey
            ...
            Case "Update"
                ...
                If ASCMAIN1.DBS_SERVER = "INT" Or ASCMAIN1.DBS_COMPANY = "INT" Then
                    ...
                    ' ARTCUST2 HAS SAME CHECKS

                    'Dim rx As String = "^([A-Z0-9]+)$" ' Allow Upper case, numbers
                    Dim rx As String = "[^a-zA-Z0-9 '#/.&@:;,+_()-]" ' Allow Upper/Lower case, numbers, space, dot, comma and dash, single-quote, hash
                    '"[^a-zA-Z0-9 .,$'#-]"
                    Dim r As New System.Text.RegularExpressions.Regex(rx)

                    'Dim SQL As String = "
                    '    Select CUST_CODE, CUST_STORE_NO, 'CUST_STORE_NAME' COL, CUST_STORE_NAME VALUE FROM ARTCUST2 WHERE CUST_CODE IN (SELECT DISTINCT CUST_CODE FROM EDTTRPM1 ) UNION
                    '    Select CUST_CODE, CUST_STORE_NO, 'CUST_STORE_ADDR1' COL, CUST_STORE_ADDR1 VALUE FROM ARTCUST2 WHERE CUST_CODE IN (SELECT DISTINCT CUST_CODE FROM EDTTRPM1 ) UNION
                    '    Select CUST_CODE, CUST_STORE_NO, 'CUST_STORE_ADDR2' COL, CUST_STORE_ADDR2 VALUE FROM ARTCUST2 WHERE CUST_CODE IN (SELECT DISTINCT CUST_CODE FROM EDTTRPM1 ) UNION
                    '    Select CUST_CODE, CUST_STORE_NO, 'CUST_STORE_ADDR3' COL, CUST_STORE_ADDR3 VALUE FROM ARTCUST2 WHERE CUST_CODE IN (SELECT DISTINCT CUST_CODE FROM EDTTRPM1 ) UNION
                    '    Select CUST_CODE, CUST_STORE_NO, 'CUST_STORE_CITY' COL, CUST_STORE_CITY VALUE FROM ARTCUST2 WHERE CUST_CODE IN (SELECT DISTINCT CUST_CODE FROM EDTTRPM1 ) UNION
                    '    Select CUST_CODE, CUST_STORE_NO, 'CUST_STORE_STATE' COL, CUST_STORE_STATE VALUE FROM ARTCUST2 WHERE CUST_CODE IN (SELECT DISTINCT CUST_CODE FROM EDTTRPM1 ) UNION
                    '    Select CUST_CODE, CUST_STORE_NO, 'CUST_STORE_ZIP_CODE' COL, CUST_STORE_ZIP_CODE VALUE FROM ARTCUST2 WHERE CUST_CODE IN (SELECT DISTINCT CUST_CODE FROM EDTTRPM1 ) UNION
                    '    Select CUST_CODE, CUST_STORE_NO, 'CUST_STORE_COUNTRY' COL, CUST_STORE_COUNTRY VALUE FROM ARTCUST2 WHERE CUST_CODE IN (SELECT DISTINCT CUST_CODE FROM EDTTRPM1 ) UNION
                    '    Select CUST_CODE, CUST_STORE_NO, 'CUST_STORE_CONTACT' COL, CUST_STORE_CONTACT VALUE FROM ARTCUST2 WHERE CUST_CODE IN (SELECT DISTINCT CUST_CODE FROM EDTTRPM1 ) UNION
                    '    Select CUST_CODE, CUST_STORE_NO, 'CUST_STORE_CONTACT' COL, CUST_STORE_CONTACT VALUE FROM ARTCUST2 WHERE CUST_CODE IN (SELECT DISTINCT CUST_CODE FROM EDTTRPM1 ) UNION
                    '    Select CUST_CODE, CUST_STORE_NO, 'CUST_STORE_LOCATION' COL, CUST_STORE_LOCATION VALUE FROM ARTCUST2 WHERE CUST_CODE IN (SELECT DISTINCT CUST_CODE FROM EDTTRPM1 ) UNION
                    '    Select CUST_CODE, CUST_STORE_NO, 'CUST_STORE_MARK_FOR' COL, CUST_STORE_MARK_FOR VALUE FROM ARTCUST2 WHERE CUST_CODE IN (SELECT DISTINCT CUST_CODE FROM EDTTRPM1 ) UNION
                    '"
                    For Each row As DataRow In dst.Tables("ARTCUST2").Select("")
                        ...
                        Dim CUST_STORE_ADDR1 As String = row.Item("CUST_STORE_ADDR1") & ""
                        Dim CUST_STORE_ADDR2 As String = row.Item("CUST_STORE_ADDR2") & ""
                        Dim CUST_STORE_ADDR3 As String = row.Item("CUST_STORE_ADDR3") & ""
                        Dim CUST_STORE_CITY As String = row.Item("CUST_STORE_CITY") & ""
                        Dim CUST_STORE_STATE As String = row.Item("CUST_STORE_STATE") & ""
                        Dim CUST_STORE_ZIP_CODE As String = row.Item("CUST_STORE_ZIP_CODE") & ""
                        Dim CUST_STORE_COUNTRY As String = row.Item("CUST_STORE_COUNTRY") & ""
                        Dim CUST_STORE_CONTACT As String = row.Item("CUST_STORE_CONTACT") & ""
                        Dim CUST_STORE_EMAIL As String = row.Item("CUST_STORE_EMAIL") & ""
                        Dim CUST_STORE_LOCATION As String = row.Item("CUST_STORE_LOCATION") & ""
                        Dim CUST_STORE_MARK_FOR As String = row.Item("CUST_STORE_MARK_FOR") & ""

                        If r.IsMatch(CUST_STORE_NAME) Then
                            EMsg &= vbCr & "Store " & CUST_STORE_NO & " Name has Special Characters which are Not allowed"
                        End If
                        If r.IsMatch(CUST_STORE_ADDR1) Then
                            EMsg &= vbCr & "Store " & CUST_STORE_NO & " Address 1 has Special Characters which are Not allowed"
                        End If
                        If r.IsMatch(CUST_STORE_ADDR2) Then
                            EMsg &= vbCr & "Store " & CUST_STORE_NO & " Address 2 has Special Characters which are Not allowed"
                        End If
                        If r.IsMatch(CUST_STORE_ADDR3) Then
                            EMsg &= vbCr & "Store " & CUST_STORE_NO & " Address 3 has Special Characters which are Not allowed"
                        End If
                        If r.IsMatch(CUST_STORE_CITY) Then
                            EMsg &= vbCr & "Store " & CUST_STORE_NO & " City has Special Characters which are Not allowed"
                        End If

                        If r.IsMatch(CUST_STORE_STATE) Then
                            EMsg &= vbCr & "Store " & CUST_STORE_NO & " State has Special Characters which are Not allowed"
                        End If
                        If r.IsMatch(CUST_STORE_ZIP_CODE) Then
                            EMsg &= vbCr & "Store " & CUST_STORE_NO & " Zip Code has Special Characters which are Not allowed"
                        End If

                        If r.IsMatch(CUST_STORE_COUNTRY) Then
                            EMsg &= vbCr & "Store " & CUST_STORE_NO & " Country has Special Characters which are Not allowed"
                        End If
                        If r.IsMatch(CUST_STORE_CONTACT) Then
                            EMsg &= vbCr & "Store " & CUST_STORE_NO & " Contact has Special Characters which are Not allowed"
                        End If
                        If r.IsMatch(CUST_STORE_EMAIL) Then
                            EMsg &= vbCr & "Store " & CUST_STORE_NO & " email has Special Characters which are Not allowed"
                        End If
                        If r.IsMatch(CUST_STORE_LOCATION) Then
                            EMsg &= vbCr & "Store " & CUST_STORE_NO & " Location has Special Characters which are Not allowed"
                        End If
                        If r.IsMatch(CUST_STORE_MARK_FOR) Then
                            EMsg &= vbCr & "Store " & CUST_STORE_NO & " Mark For has Special Characters which are Not allowed"
                        End If
                    Next
    ...
    Public Overrides Sub tlb_ToolClick(ByVal sender As System.Object, ByVal e As Infragistics.Win.UltraWinToolbars.ToolClickEventArgs)
        ...
        Select Case e.Tool.Key
            ...
            Case "Copy Value and Paste to All Stores"
                If grd.ActiveCell Is Nothing Then Exit Sub
                Dim COLUMN_NAME As String = grd.ActiveCell.Column.Key
                If COLUMN_NAME = "" Then Exit Sub
...
#Region "grdARTCUST2"
    ...
    Private Sub grdARTCUST2_BeforeRowUpdate(ByVal sender As Object, ByVal e As Infragistics.Win.UltraWinGrid.CancelableRowEventArgs) Handles grdARTCUST2.BeforeRowUpdate
        ...
        Dim SHIP_VIA_CODE As String = e.Row.Cells("CUST_STORE_SHIP_VIA_CODE").Value & String.Empty
        If SHIP_VIA_CODE.Length > 0 Then
            ...
            If rowSOTSVIA1 IsNot Nothing Then
                ' Done to get proper case
                e.Row.Cells("CUST_STORE_SHIP_VIA_CODE").Value = rowSOTSVIA1.Item("SHIP_VIA_CODE")
            Else
    ...
    Private Sub grdARTCUST2_ClickCellButton(ByVal sender As Object, ByVal e As Infragistics.Win.UltraWinGrid.CellEventArgs) Handles grdARTCUST2.ClickCellButton
        ...
        Dim COLUMN_NANME As String = String.Empty
        Dim VIEW_NAME As String = String.Empty

        Select Case grdARTCUST2.ActiveCell.Column.Key
            Case "SELL_CODE"

            Case "CUST_STORE_SHIP_VIA_CODE"
                VIEW_NAME = "SHIP_VIA_CODE.SOTSVIA1"
        End Select

        grdClickCellButton(grdARTCUST2, sql_where, False, COLUMN_NAME, VIEW_NAME)
    End Sub
