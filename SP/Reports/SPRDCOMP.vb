Public Class SPRDCOMP

#Region "General Declarations"
    Dim SPTDPMT1 As String
...
#End Region
    ...
    Protected Overrides Sub Build_Workfile()
        '  RWU = "R"
        Dim sqlw As String = ""
        sqlw &= "   and SPTDPMT1.OPS_YYYYPP >= '" & RYP0 & "' and SPTDPMT1.OPS_YYYYPP <= '" & RYP1 & "'"
        If CHKMTD = "1" Then
            sqlw &= "   and NVL(SPTDPMT1.JOURNAL_IND,'0') = '0'" '  is Null"
        End If
        sqlw &= SQL_in("CUST_CODE", "SPTDPMT1.CUST_CODE") & vbCrLf
        sqlw &= SQL_in("PYMT_NO", "SPTDPMT1.PYMT_NO") & vbCrLf
        Prepare_dst(True, sqlw)
        Check_if_Empty("SPTDPMT1")
    End Sub
    ...
    Overrides Sub Update_Record()
        ...
        ASCMAIN1.sql = "Update SPTDPMT1 set JOURNAL_IND = '1', JOURNAL_XNO = '" & XNO & "'" & vbCrLf _
            & " where PYMT_NO in (Select PYMT_NO from " & SPTDPMT1 & ")"
        ASCDATA1.ExecuteSQL(sql)

        GL_Update()

        'Else
        '    ASCMAIN1.sql = "Update SPTDPMT1 set REGISTER_IND = '1', REGISTER_XNO = '" & XNO & "'" & vbCrLf _
        '        & " where PYMT_NO in (Select PYMT_NO from " & SPTDPMT1 & ")"
        '    ASCDATA1.ExecuteSQL(sql)
        'End If
    ...
    ByVal ParamArray parms() As Object) As ASCBASE1
        ...
        With dst
            ...
            ASCMAIN1.sql = "Select SPTDPMT1.* from " & SPTDPMT1 & " SPTDPMT1"
            Create_TDA(.Tables.Add, "SPTDPMT1", "**", 0, False, "", 1)

            ASCMAIN1.sql = "Select SPTDPMT2.*,ARTCUST1.TRADE_CLASS_CODE,SPTDPMT1.OPS_YYYYPP" & vbCrLf _
                & " from SPTDPMT2, " & SPTDPMT1 & " SPTDPMT1, ARTCUST1" & vbCrLf _
                & " where SPTDPMT2.PYMT_NO = SPTDPMT1.PYMT_NO" & vbCrLf _
                & "   and ARTCUST1.CUST_CODE = SPTDPMT1.CUST_CODE"
            Create_TDA(.Tables.Add, "SPTDPMT2", "**", 0, False, "", 3)

            Create_Relation("SPTDPMT1", "SPTDPMT2", "PYMT_NO")

            With .Tables("SPTDPMT2")
                .Columns.Add("CUST_CODE", GetType(System.String), "PARENT.CUST_CODE")
                .Columns.Add("PYMT_REF_NO", GetType(System.String), "PARENT.PYMT_REF_NO")
            End With

            ASCMAIN1.sql = "Select SPTDCOMC.* from SPTDCOMC, " & SPTDPMT1 & " SPTDPMT1" & vbCrLf _
                & " where SPTDCOMC.PYMT_NO = SPTDPMT1.PYMT_NO"
            Create_TDA(.Tables.Add, "SPTDCOMC", "**", 0, False, "", 1)
    ...
    Public Overrides Sub Fill_Records_RPT(ByVal ParamArray parms() As Object)
        ...
        ASCDATA1.ExecuteSQL("Truncate Table " & SPTDPMT1)
        ASCMAIN1.sql = "Select * from SPTDPMT1 " & ASCMAIN1.SQL_Add_WHERE(sqlw)
        ASCDATA1.ExecuteSQL("Insert into " & SPTDPMT1 & " " & ASCMAIN1.sql)

        EnforceConstraints(False)
        Fill_Records("SPTDPMT1")
        Fill_Records("SPTDPMT2")
        Fill_Records("SPTDCOMC")
        EnforceConstraints(True)

        'If CHKMTD = "1" Then
        Prepare_GL_Interface("SPDP")
        'End If
    End Sub
    ...
    Function Prepare_GL_Interface(ByVal JOURNAL_TYPE As String) As String
        ...
        For Each rowYP As DataRow In _
            ASCDATA1.SelectDistinct(dst.Tables("SPTDPMT2"), New String() {"OPS_YYYYPP"}).Select
            ...
            For Each row As DataRow In
                ASCDATA1.SelectDistinct(dst.Tables("SPTDPMT2").Select("OPS_YYYYPP = '" & OPS_YYYYPP & "'"),
                                        New String() {"TRADE_CLASS_CODE", "COLLECTION_CODE", "CUST_CODE", "PYMT_REF_NO"}).Select
                ...
                Dim PYMT_REF_NO As String = row.Item("PYMT_REF_NO")

                Dim sqlw As String = "TRADE_CLASS_CODE = '" & TRADE_CLASS_CODE & "' and COLLECTION_CODE = '" & COLLECTION_CODE & "' and OPS_YYYYPP = '" & OPS_YYYYPP & "'"
                sqlw &= $" and CUST_CODE = '{CUST_CODE}' and PYMT_REF_NO = '{PYMT_REF_NO}'"
                Dim DETL_POSTING_AMT As Decimal = Val(dst.Tables("SPTDPMT2").Compute("SUM(AMT_COMM_ADJ)", sqlw) & "")
                ...
                If DETL_POSTING_AMT <> 0 Then
                    ...
                    rowGLTINTF1.Item("DETL_CVX_REF_NO") = PYMT_REF_NO
                    rowGLTINTF1.Item("DETL_DESC") = CUST_CODE

                End If
            Next

            ' Accrual
            ' Dim DEMO_COMM_OFFSET As Decimal = Val(dst.Tables("SPTDCOMC").Compute("SUM(DEMO_COMM_OFFSET)", "") & "")
            Dim AMT_COMM_OFFSET As Decimal = Val(dst.Tables("SPTDCOMC").Compute("SUM(AMT_COMM_OFFSET)", "OPS_YYYYPP_PAID = '" & OPS_YYYYPP & "'") & "")
            ...
            Dim rowSOTTYPE1 As DataRow = LookUp("SOTTYPE1", ROWs("SPTPARM1").Item("SP_PARM_DEMO_TYPE_CODE"))
            Dim rowARTPOST1 As DataRow = dst.Tables("ARTPOST1").Rows.Find(rowSOTTYPE1.Item("POST_CODE"))
            Dim AMT_COMM_PAID As Decimal = Val(dst.Tables("SPTDCOMC").Compute("SUM(AMT_COMM_PAID)", "OPS_YYYYPP_PAID = '" & OPS_YYYYPP & "'") & "")
    ...
    Sub Create_Work_Tables()
        ...
        ASCMAIN1.sql = "Select * from SPTDPMT1 where ROWNUM < 1"
        SPTDPMT1 = ASCMAIN1.Temp_Table
        ASCDATA1.ExecuteSQL("Alter Table " & SPTDPMT1 & " Add Primary Key (PYMT_NO)")
