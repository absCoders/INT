Public Class EDC855O1
    ...
    Private Const Cancel850 As String = "Cancel850"

    Public Shared Function CreateEDTSYSIH(ByRef dst As DataSet,
    ...
    Public Shared Sub Generate_855_Cancelling_850(ByVal clsASCBASE1 As ASCBASE1, ByVal EDI_DOC_SEQ_NO_IN As String)
        ...
        If Not clsASCBASE1.dst.Tables.Contains("EDT850T1_850") Then
            clsASCBASE1.Create_TDA(clsASCBASE1.dst.Tables.Add("EDT850T1_850"), "EDT850T1", "*")
            clsASCBASE1.Create_TDA(clsASCBASE1.dst.Tables.Add("EDT850T2_850"), "EDT850T2", "*", 1)
            clsASCBASE1.Create_TDA(clsASCBASE1.dst.Tables.Add("EDT850T3_850"), "EDT850T3", "*", 1)
            clsASCBASE1.Create_TDA(clsASCBASE1.dst.Tables.Add("EDT850T4_850"), "EDT850T4", "*", 1)
            clsASCBASE1.Create_TDA(clsASCBASE1.dst.Tables.Add("EDT850T5_850"), "EDT850T5", "*", 1)
        Else
            clsASCBASE1.dst.Tables("EDT850T1_850").Rows.Clear()
            clsASCBASE1.dst.Tables("EDT850T2_850").Rows.Clear()
            clsASCBASE1.dst.Tables("EDT850T3_850").Rows.Clear()
            clsASCBASE1.dst.Tables("EDT850T4_850").Rows.Clear()
            clsASCBASE1.dst.Tables("EDT850T5_850").Rows.Clear()
        End If

        For Each tableName As String In New String() {"EDT850T1_850", "EDT850T2_850", "EDT850T3_850", "EDT850T4_850", "EDT850T5_850"}
            clsASCBASE1.Fill_Records(tableName, EDI_DOC_SEQ_NO_IN)
        Next

        Dim rowEDT850T1 As DataRow = clsASCBASE1.dst.Tables("EDT850T1_850").Rows.Find(EDI_DOC_SEQ_NO_IN)
        If rowEDT850T1 Is Nothing Then
            MessageBox.Show($"Cannot locate EDI 850 {EDI_DOC_SEQ_NO_IN}.", "Generate 855", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Exit Sub
        End If

        Dim EDI_TP_QUAL As String = rowEDT850T1.Item("EDI_TP_QUAL") & String.Empty
        Dim EDI_TP_ID As String = rowEDT850T1.Item("EDI_TP_ID") & String.Empty
        Dim CUST_CODE As String = rowEDT850T1.Item("CUST_CODE") & String.Empty
        ...
        ASCMAIN1.sql = $"Select * from EDTTRPM1 Where EDI_DOC_NO = '855' And EDI_TP_QUAL = '{EDI_TP_QUAL}' And EDI_TP_ID = '{RTrim(EDI_TP_ID)}'"
        ...
        Dim EDI_PURP_CODE As String = "00"
        Dim NUMBER_CHARS_STORE As Int32 = 4
        Dim NUMBER_CHARS_DC As Int32 = 4

        Dim tblEDTSLSP1 As DataTable = ASCDATA1.GetDataTable($"SELECT * FROM EDTSLSP1 WHERE CUST_CODE = '{CUST_CODE}'")
        ...
        Dim rowEDT855O1 As DataRow = clsASCBASE1.dst.Tables("EDT855O1").NewRow
        With rowEDT855O1
            .Item("COMPANY_CODE") = ASCMAIN1.DBS_COMPANY
            .Item("EDI_OUTBOUND_DOC_NO") = EDI_Outbound_Doc_No
            .Item("ORDR_CUST_PO") = rowEDT850T1.Item("EDI_PO_NO")
            ...
            .Item("TERM_CODE") = rowEDT850T1.Item("EDI_TERM_TYPE")
            .Item("ORDR_SHIP_DATE") = rowEDT850T1.Item("EDI_SHIP_DATE")
            .Item("ORDR_CANCEL_DATE") = rowEDT850T1.Item("EDI_END_DATE")
            .Item("AS_OF_DATE") = Format(Now, "dd-MMM-yy")
            .Item("INIT_DATE") = Format(Now, "dd-MMM-yy")
            .Item("INIT_OPER") = ASCMAIN1.USER_ID
            .Item("EDI_PURPOSE_CODE") = EDI_PURP_CODE
            .Item("EDI_ARRIVAL_DATE") = rowEDT850T1.Item("EDI_ARRIVAL_DATE")

            .Item("EDI_ACK_TYPE") = Ack_Type
            .Item("EDI_SUPPLIER_NO") = rowEDT850T1.Item("EDI_SUPPLIER_NO")
            .Item("EDI_DEPARTMENT") = rowEDT850T1.Item("EDI_DEPARTMENT")
            .Item("EDI_SHIP_DC") = rowEDT850T1.Item("EDI_SHIP_DC")

            .Item("EDI_START_DATE_ORIG") = rowEDT850T1.Item("EDI_START_DATE")
            .Item("EDI_END_DATE_ORIG") = rowEDT850T1.Item("EDI_END_DATE")
            .Item("EDI_SHIP_DATE_ORIG") = rowEDT850T1.Item("EDI_SHIP_DATE")
            .Item("EDI_ARRIVAL_DATE_ORIG") = rowEDT850T1.Item("EDI_ARRIVAL_DATE")
        End With

        clsASCBASE1.dst.Tables("EDT855O1").Rows.Add(rowEDT855O1)

        ASCMAIN1.sql = $"Select * from EDT850T2 Where EDI_DOC_SEQ_NO = '{EDI_DOC_SEQ_NO_IN}'"
        For Each rowEDT850T2 As DataRow In ASCDATA1.GetDataTable.Select("", "EDI_DTL_SEQ")
            If Add_EDT855O2(clsASCBASE1, rowEDT850T2, Cancel850, EDI_Outbound_Doc_No, EDI_TP_ID, False, String.Empty) Then
                Add_EDT85503_Cancel_850(clsASCBASE1, rowEDT850T2, EDI_Outbound_Doc_No)
            End If
        Next

        ASCMAIN1.sql = $"Select * from EDT850T5 where EDI_DOC_SEQ_NO = '{EDI_DOC_SEQ_NO_IN}' And EDI_ADDR_TYPE <> 'SF' "
        ...
        ASCMAIN1.sql = $"Select * from EDT850T7 where EDI_DOC_SEQ_NO = '{EDI_DOC_SEQ_NO_IN}'"
    ...
    Public Shared Sub Generate_855(ByVal clsASCBASE1 As ASCBASE1,
        ...
        If Not clsASCBASE1.dst.Tables.Contains("SOTORDR1_850") Then
            clsASCBASE1.Create_TDA(clsASCBASE1.dst.Tables.Add("SOTORDR1_850"), "SOTORDR1", "*")
            clsASCBASE1.Create_TDA(clsASCBASE1.dst.Tables.Add("SOTORDR2_850"), "SOTORDR2", "*")
        Else
            clsASCBASE1.dst.Tables("SOTORDR1_850").Rows.Clear()
            clsASCBASE1.dst.Tables("SOTORDR2_850").Rows.Clear()
        End If
        ...
        If WHSE_CODE.Length > 0 Then
            ASCMAIN1.sql &= $" AND R1.WHSE_CODE = '{WHSE_CODE}'"
        End If
        ...
        ' Need these since not all screens calling this have Full SOTORDR1 and SOTORDR2 in the datset.
        ASCMAIN1.sql = $"SELECT * FROM SOTORDR1 WHERE ORDR_GROUP_NO = '{ORDR_GROUP_NO}'"
        If WHSE_CODE.Length > 0 Then
            ASCMAIN1.sql &= $" AND WHSE_CODE = '{WHSE_CODE}'"
        End If
        clsASCBASE1.Fill_Records("SOTORDR1_850", String.Empty, True, ASCMAIN1.sql)

        ASCMAIN1.sql = $"SELECT SOTORDR2.*, SOTORDR1.ORDR_GROUP_NO 
        ...
        If WHSE_CODE.Length > 0 Then
            ASCMAIN1.sql &= $" AND SOTORDR1.WHSE_CODE = '{WHSE_CODE}'"
        End If
        clsASCBASE1.Fill_Records("SOTORDR2_850", String.Empty, True, ASCMAIN1.sql)
        ...
        For Each rowEDT850T2 As DataRow In ASCDATA1.GetDataTable.Rows
            ...
            If WHSE_CODE.Length > 0 Then
                Dim inEDI_DOC_SEQ_NO As String = rowEDT850T2.Item("EDI_DOC_SEQ_NO") & String.Empty
                Dim inEDI_DTL_SEQ As Int16 = Val(rowEDT850T2.Item("EDI_DTL_SEQ") & String.Empty)

                Dim query As String = $"EDI_DOC_SEQ_NO = '{inEDI_DOC_SEQ_NO}' and EDI_DTL_SEQ = {inEDI_DTL_SEQ}"
                If clsASCBASE1.dst.Tables("SOTORDR2_850").Select(query).Length = 0 Then
                    Continue For
                End If
            End If

            If Add_EDT855O2(clsASCBASE1, rowEDT850T2, ORDR_GROUP_NO, EDI_Outbound_Doc_No, EDI_TP_ID, REVERSE_PO, WHSE_CODE) Then
                Add_EDT855O3(clsASCBASE1, rowEDT850T2, ORDR_GROUP_NO, EDI_Outbound_Doc_No, WHSE_CODE)
            End If
        Next
    ...
    Public Shared Function Add_EDT855O2(ByVal clsASCBASE1 As ASCBASE1,
                  ByVal rowEDT850T2 As DataRow,
                  ByVal ORDR_GROUP_NO As String,
                  ByVal EDI_Outbound_Doc_No As String,
                  ByVal EDI_TP_ID As String,
                  ByVal REVERSE_PO As Boolean,
                  ByVal WHSE_CODE As String) As Boolean
        ...
        With rowEDT855O2
            ...
            If ORDR_GROUP_NO <> Cancel850 Then
                ASCMAIN1.sql = $"Select MAX(ORDR_UNIT_PRICE) ORDR_UNIT_PRICE, 
                                SUM(DECODE(SOTORDR1.ORDR_STATUS, 'O', 0, ORDR_QTY_OPEN)) ORDR_QTY_OPEN, 
                                SUM(DECODE(SOTORDR1.ORDR_STATUS, 'O', ORDR_QTY_OPEN, ORDR_QTY_PICK)) ORDR_QTY_PICK, 
                ...
                If WHSE_CODE.Length > 0 Then
                    ASCMAIN1.sql &= $" AND SOTORDR1.WHSE_CODE = '{WHSE_CODE}'"
                End If
                rowSOTORDR2 = ASCDATA1.GetDataRow
            End If
            ...
            If ORDR_GROUP_NO = Cancel850 Then
                .Item("EDI_PRICE_ACTUAL") = rowEDT850T2.Item("EDI_PRICE")
                .Item("EDI_QTY_OPEN") = 0
                .Item("EDI_QTY_PICK") = 0
                .Item("EDI_QTY_CANC") = rowEDT850T2.Item("EDI_TOTAL_QTY")
            Else
    ...
    Public Shared Sub Add_EDT855O3(ByVal clsASCBASE1 As ASCBASE1,
               ByVal rowEDT850T2 As DataRow,
               ByVal ORDR_GROUP_NO As String,
               ByVal EDI_Outbound_Doc_No As String,
               ByVal WHSE_CODE As String)
        ...
        Dim query As String = $"ORDR_GROUP_NO = '{ORDR_GROUP_NO}'"
        If WHSE_CODE.Length > 0 Then
            query &= $" and WHSE_CODE = '{WHSE_CODE}'"
        End If

        For Each rowSOTORDR1 As DataRow In clsASCBASE1.dst.Tables("SOTORDR1_850").Select(query)
            ...
            If Not clsASCBASE1.dst.Tables("SOTORDR2_850").Columns.Contains("ORDR_GROUP_NO") Then
                Select Case rowSOTORDR1.Item("ORDR_STATUS") & String.Empty
                    Case "O"
                        sql = "ORDR_NO = '" & ORDR_NO & "'" _
                            & " AND ORDR_QTY_OPEN > 0" _
            ...
            For Each rowSOTORDR2 As DataRow In clsASCBASE1.dst.Tables("SOTORDR2_850").Select(sql, "CUST_STORE_NO")
    ...
    Public Shared Sub Add_EDT85503_Cancel_850(ByVal clsASCBASE1 As ASCBASE1, ByVal rowEDT850T2 As DataRow, ByVal EDI_Outbound_Doc_No As String)

        Dim EDI_DOC_SEQ_NO As String = rowEDT850T2.Item("EDI_DOC_SEQ_NO") & String.Empty
        Dim EDI_DTL_SEQ As Int64 = Val(rowEDT850T2.Item("EDI_DTL_SEQ") & String.Empty)

        Dim rowEDT855O3 As DataRow = Nothing

        For Each rowEDT850T3 As DataRow In clsASCBASE1.dst.Tables("EDT850T3").Select($"EDI_DOC_SEQ_NO = '{EDI_DOC_SEQ_NO}' and EDI_DTL_SEQ = {EDI_DTL_SEQ}", "EDI_SDQ_SEQ")
            rowEDT855O3 = clsASCBASE1.dst.Tables("EDT855O3").NewRow
            rowEDT855O3.Item("COMPANY_CODE") = ASCMAIN1.DBS_COMPANY
            rowEDT855O3.Item("EDI_OUTBOUND_DOC_NO") = EDI_Outbound_Doc_No
            rowEDT855O3.Item("EDI_DTL_SEQ") = rowEDT850T2.Item("EDI_DTL_SEQ")
            rowEDT855O3.Item("EDI_SDQ_SEQ") = rowEDT850T3.Item("EDI_SDQ_SEQ")
            rowEDT855O3.Item("EDI_SDQ_UOM") = rowEDT850T2.Item("EDI_PO4_UOM")
            'rowEDT855O3.Item("EDI_SDQ_QUAL") = String.Empty

            For iLoop As Int16 = 1 To 10
                rowEDT855O3.Item("EDI_STORE_" & iLoop.ToString.PadLeft(2, "0")) = rowEDT850T3.Item("EDI_STORE_" & iLoop.ToString.PadLeft(2, "0"))
                rowEDT855O3.Item("EDI_QTY_" & iLoop.ToString.PadLeft(2, "0")) = rowEDT850T3.Item("EDI_QTY_" & iLoop.ToString.PadLeft(2, "0"))
            Next
