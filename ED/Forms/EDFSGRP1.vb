...
#Region "ABS Standard Routines" ' These Routines should be found in all Forms which Launch from the Menu.
    ...
    Private Sub Form_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        With dst
            ...
            With .Tables("GMTCGMAX").Columns
                ...
                .Add("SSX", GetType(System.Int32))
                .Add("ADJ", GetType(System.Int32))
                .Add("QTY_OPN_1", GetType(System.Int32))
                .Add("QTY_OPN_2", GetType(System.Int32))
            End With
    ...
    Sub Print_Record()
        ...
        Dim reportFile As String = "MARSCOL1"
       
        Me.Cursor = Cursors.WaitCursor
        ASCMAIN1.Progress("Now Preparing " & RPT_TITLE)

        Print_Report_Begin()
        CR_params.Add("SUBT", STYLE_GROUP_DESC)
    ...
    Sub Load_Record()
        ...
        optIC.Tag = ""
        Setup_grdGMTCGMAX()
        grdMATSGRP1.Text = "Collection " & STYLE_GROUP_DESC & ": Styles"
...
#End Region
    ...
    Sub Add_Style(DGC_CODE As String, VEND_CODE As String, STYLE_CODE As String, COLOR_CODE As String)
        ...
        If rowGMTSTYL1 IsNot Nothing Then
            rowMATSGRP1 = dst.Tables("MATSGRP1").NewRow
                ...
                For Each dcol As DataColumn In dst.Tables("MATSGRP1").Columns
                    ...
                    Else
                        If row(0).Table.Columns.Contains(dcol.ColumnName) Then
                            rowMATSGRP1.Item(dcol.ColumnName) = row(0).Item(dcol.ColumnName)
                        End If
                    End If
                Next
            End If
        ...
        Else
            ...
            If optIC.Value = "A" Then
                ...
                ASCMAIN1.sql = "Select '*' DGC_CODE, '*' VEND_CODE, '*' STYLE_CODE, '*' COLOR_CODE, STORE_NO" _
                    ...
                    & " group by STORE_NO"
            Else
                grdGMTCGMAX.Text = "Collection " & STYLE_GROUP_DESC & ": Sales & On Hand - DGC " & DGC_CODE & ", Vendor " & VEND_CODE & ", Style " & STYLE_CODE & ", Color " & COLOR_CODE
            ...
            If optIC.Value <> "A" Or optIC.Tag & "" <> "A" Then
                ...
                ASCMAIN1.sql = "Select Sum (CASE WHEN POTORDR1.PO_CANCEL_DATE <= SYSDATE + 5 * 7 THEN QTY_OPN ELSE 0 END) QTY_OPN_1" _
                    & " , Sum (CASE WHEN POTORDR1.PO_CANCEL_DATE > SYSDATE + 5 * 7 THEN QTY_OPN ELSE 0 END) QTY_OPN_2" _
                    & " from POTORDR1,POTORDR2," & MATSGRP1 & " MATSGRP1" _
                    & " where POTORDR1.PO_ORDER_NO = POTORDR2.PO_ORDER_NO" _
                    & " and POTORDR2.DGC_CODE = MATSGRP1.DGC_CODE and POTORDR2.STYLE_CODE = MATSGRP1.STYLE_CODE and POTORDR2.COLOR_CODE = MATSGRP1.COLOR_CODE and POTORDR1.VEND_CODE = MATSGRP1.VEND_CODE"

                If optIC.Value = "A" Then
                Else
                    ASCMAIN1.sql &= " and POTORDR2.DGC_CODE = :PARM1 and POTORDR2.STYLE_CODE = :PARM2 and POTORDR2.COLOR_CODE = :PARM3 and POTORDR1.VEND_CODE = :PARM4"
                End If
    ...
    Private Sub grdMATSGRP1_AfterRowsDeleted(sender As Object, e As System.EventArgs) Handles grdMATSGRP1.AfterRowsDeleted
        Setup_grdGMTCGMAX()
    End Sub

    Private Sub grdMATSGRP1_BeforeRowsDeleted(sender As Object, e As Infragistics.Win.UltraWinGrid.BeforeRowsDeletedEventArgs) Handles grdMATSGRP1.BeforeRowsDeleted
        For Each GROW As UltraWinGrid.UltraGridRow In e.Rows
            Dim DGC_CODE As String = GROW.Cells("DGC_CODE").Value
            Dim VEND_CODE As String = GROW.Cells("VEND_CODE").Value
            Dim STYLE_CODE As String = GROW.Cells("STYLE_CODE").Value
            Dim COLOR_CODE As String = GROW.Cells("COLOR_CODE").Value

            ASCDATA1.ExecuteSQL("Delete from " & MATSGRP1 & " where DGC_CODE = :PARM1 and VEND_CODE = :PARM2 and STYLE_CODE = :PARM3 and COLOR_CODE = :PARM4")
            ASCDATA1.ExecuteSQL(ASCMAIN1.sql, "VVVV", New Object() {DGC_CODE, VEND_CODE, STYLE_CODE, COLOR_CODE})
        Next
        optIC.Tag = ""
