...
#Region "ABS Standard Routines" ' These Routines should be found in all Forms which Launch from the Menu.
    ...
    Private Sub Form_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        With dst
            ...
            .Tables("SOTORDR0").Columns.Add("ORDR_HOLD")
            .Tables("SOTORDR0").Columns.Add("ORDR_OVERRIDE_NOT_ALLOCATED")
            .Tables("SOTORDR0").Columns.Add("REASON_CODE")
            .Tables("SOTORDR0").Columns.Add("ORDR_ADDR_TYPE_ST")
        ...
        Create_Summary(grdSOTORDRD, "MLNO", "Count")
        Create_Summary(grdSOTORDRD, New String() {"ORDR_QTY", "ORDR_QTY_OPEN", "ORDR_QTY_PICK", "ORDR_QTY_SHIP", "ORDR_QTY_CANC"})

        Create_Summary(grdSOTORDRC, "ORDR_LNO", "Count")
        Create_Summary(grdSOTORDRC, New String() {"ORDR_QTY", "ORDR_QTY_OPEN", "ORDR_QTY_PICK", "ORDR_QTY_SHIP", "ORDR_QTY_CANC"})
        ...
        tabMultiOrder.Tabs("DC").Visible = False
        tabMultiOrder.Tabs("Store x Item").Visible = False
    End Sub
    ...
    Overrides Sub Proceed_PreReq(ByVal eItemKey As String)
        ...
        Select Case eItemKey
            ...
            Case "Mass Change to Orders"
                If Absx1.txtFor("CUST_CODE").Text = "" Or CUST_CODE = "" Then
                    EMsg &= vbCr & "You Must First Specify a Valid Customer"
                End If
                ...
                If EMsg = "" Then
                    ...
                    If ASCMAIN1.CodeSelector.SQL <> "" Then
                        ...
                        If ASCMAIN1.CodeSelector.SelectedCode <> "" Then
                            Dim ORDR_GROUP_NO As String = ASCMAIN1.CodeSelector.SelectedCode
                            Dim CUST_CODE As String = ASCMAIN1.CodeSelector.SelectedRows(0).Item("CUST_CODE")
                            If Not ASCMAIN1.Logical_Lock("SOTORDR0", ORDR_GROUP_NO) Then
                                Exit Sub
                            Else
                                'Dim CUST_CODE As String = ASCDATA1.GetDataValue _
                                '    ("Select CUST_CODE from SOTORDR0 where ORDR_GROUP_NO = '" & ORDR_GROUP_NO & "'")
                                If Not ASCMAIN1.Logical_Lock("SOTORDR1", CUST_CODE) Then
                                    Exit Sub
                                End If
                            End If
                        End If
    ...
    Overrides Sub Mode_Settings(ByVal tf As Boolean, Optional ByVal MODE_description As String = "")
        ...
        With UltraExplorerBar1
            With .Groups("Screen Control")
                ...
                .Items("Multi-Order Maint").Settings.Enabled = iScreenMode
                .Items("Mass Change to Orders").Settings.Enabled = iScreenMode
                ...
                .Items("Multi-Order Maint").Visible = (EntryMode = "E")
                ' .Items("Mass Change to Orders").Visible = (EntryMode = "E")
                .Items("Cancel Order").Visible = (EntryMode = "E")

                ' UNTIL TESTED
                .Items("Multi-Order Maint").Visible = False
                .Items("Mass Change to Orders").Visible = False
    ...
    Sub Update_Record_multiple_order_group()
        ...
        For Each ORDR_GROUP_NO As String In ORDR_GROUP_NOs
            ASCMAIN1.sql = "Update SOTORDR1 " _
                & " Set ORDR_SHIP_DATE = :PARM1, ORDR_CANCEL_DATE = :PARM2, ORDR_HOLD = :PARM3, ORDR_OVERRIDE_NOT_ALLOCATED = :PARM4" _
                & ", LAST_DATE = :PARM5, LAST_OPER = :PARM6" _
                & " where ORDR_GROUP_NO = :PARM7"
            ASCDATA1.ExecuteSQL(ASCMAIN1.sql, "DDVVDVV", New Object() {ORDR_SHIP_DATE, ORDR_CANCEL_DATE, ORDR_HOLD, ORDR_OVERRIDE_NOT_ALLOCATED, DATETIME_STAMP, ASCMAIN1.USER_ID, ORDR_GROUP_NO})
...
#Region "Popup_Menus"
    ...
    Overrides Sub Load_Popup_Menus()
        ...
        Load_Popup_Menu(grdSOTORDRC, "BB", "Select All", "De-Select All")
    ...
    Public Overrides Sub tlb_ToolClick(ByVal sender As System.Object, ByVal e As Infragistics.Win.UltraWinToolbars.ToolClickEventArgs)
        ...
        Select Case e.Tool.Key
            ...
            Case "Select All", "De-Select All"
                For Each grow As UltraWinGrid.UltraGridRow In grdSOTORDRC.Rows
                    If Val(grow.Cells("ORDR_QTY_OPEN").Value & "") <> 0 Then
                        grow.Cells("SUB").Value = IIf(e.Tool.Key = "Select All", "1", "0")
                    End If
                Next
...
#Region "VB6"
    ...
    Sub Setup_MOM()
        ...
        If ASCMAIN1.Running_in_VS Then Stop ' ORDR_GORUP_NO should have a value

        Setup_MOM_Details()

        ASCMAIN1.sql = "Select " _
            & "  MIN (ORDR_SHIP_DATE) ORDR_SHIP_DATE " & vbCrLf _
            & ", MIN (ORDR_CANCEL_DATE) ORDR_CANCEL_DATE " & vbCrLf _
            & ", MIN (ORDR_DEPT) ORDR_DEPT" & vbCrLf _
            & ", MAX (ORDR_HOLD) ORDR_HOLD" & vbCrLf _
            & ", MAX (ORDR_OVERRIDE_NOT_ALLOCATED) ORDR_OVERRIDE_NOT_ALLOCATED" & vbCrLf _
            & ", MAX (ORDR_ADDR_TYPE_ST) ORDR_ADDR_TYPE_ST" & vbCrLf _
            & ", MIN (SREP_CODE) SREP_CODE " & vbCrLf _
            & ", MIN (SREP2_CODE) SREP2_CODE " & vbCrLf _
            & ", MAX (CUST_FACTOR_IND) CUST_FACTOR_IND" & vbCrLf _
            & " from SOTORDR1 where ORDR_GROUP_NO = '" & ORDR_GROUP_NO & "'"
        Dim row As DataRow = ASCDATA1.GetDataRow

        Dim rowSOTORDRB As DataRow = dst.Tables("SOTORDRB").NewRow
        With rowSOTORDRB
            .Item("ORDR_SHIP_DATE") = row.Item("ORDR_SHIP_DATE")
            .Item("ORDR_CANCEL_DATE") = row.Item("ORDR_CANCEL_DATE")
            .Item("ORDR_DEPT") = row.Item("ORDR_DEPT")
            .Item("SREP_CODE") = row.Item("SREP_CODE")
            .Item("SREP2_CODE") = row.Item("SREP2_CODE")
            .Item("ORDR_HOLD") = row.Item("ORDR_HOLD")
            .Item("ORDR_OVERRIDE_NOT_ALLOCATED") = row.Item("ORDR_OVERRIDE_NOT_ALLOCATED")
            .Item("CUST_FACTOR_IND") = row.Item("CUST_FACTOR_IND")
            .Item("ORDR_ADDR_TYPE_ST") = row.Item("ORDR_ADDR_TYPE_ST")
            If .Item("ORDR_ADDR_TYPE_ST") & "" <> "DC" Then .Item("ORDR_ADDR_TYPE_ST") = "MK"
            Absx1.optFor("SOTORDRB.ORDR_ADDR_TYPE_ST").Tag = Absx1.optFor("SOTORDRB.ORDR_ADDR_TYPE_ST").Value
        End With
        dst.Tables("SOTORDRB").Rows.Add(rowSOTORDRB)

        frmSOTORDRD.Visible = True

        Me.Cursor = Cursors.Default
        ASCMAIN1.Progress("")
    End Sub

    Sub Setup_MOM_Details()

        ASCMAIN1.sql = "Select " _
            & "SOTORDR1.ORDR_NO, SOTORDR2.ORDR_LNO, SOTORDR2.ORDR_LNO MLNO, SOTORDR1.CUST_STORE_NO" & vbCrLf _
            & ", SOTORDR2.ITEM_CODE, " & vbCrLf _
            & ", SOTORDR2.ORDR_QTY, SOTORDR2.ORDR_QTY_OPEN, SOTORDR2.ORDR_QTY_PICK" & vbCrLf _
            & ", SOTORDR2.ORDR_QTY_SHIP, SOTORDR2.ORDR_QTY_CANC, 0 SUB" & vbCrLf _
            & ", SOTORDR2.ITEM_CODE ITEM_CODE_ORIG, SOTORDR2.ORDR_QTY_OPEN ORDR_QTY_OPEN_ORIG, NULL" & vbCrLf _
            & " from SOTORDR1,SOTORDR2 " & vbCrLf _
            & " where SOTORDR2.ORDR_NO = SOTORDR1.ORDR_NO" & vbCrLf _
            & "   and SOTORDR1.ORDR_GROUP_NO = '" & ORDR_GROUP_NO & "'" & vbCrLf
        Fill_Records("SOTORDRC", "", , ASCMAIN1.sql)

        ASCMAIN1.sql = "Select ROWNUM MLNO, X.*, 0 ORDR_CXL, 0 ORDR_SUB from (Select " & vbCrLf _
            & "  SOTORDR2.ITEM_CODE" & vbCrLf _
            ...
            & " from SOTORDR2 WHERE ORDR_NO IN (SELECT ORDR_NO FROM SOTORDR1" & vbCrLf _
            & " WHERE ORDR_GROUP_NO = '" & ORDR_GROUP_NO & "')" & vbCrLf _
            & " group by SOTORDR2.ITEM_CODE, SOTORDR2.COLOR_CODE, SOTORDR2.ITEM_CODE_SUB" & vbCrLf _
            & ", SOTORDR2.CUST_ITEM_CODE, SOTORDR2.CUST_COLOR_CODE, SOTORDR2.CUST_SIZE_CODE" & vbCrLf _
            & ", SOTORDR2.CUST_UPC, SOTORDR2.CUST_SKU" & vbCrLf _
            & ") X" & vbCrLf _
            & " order by ITEM_CODE, COLOR_CODE, ITEM_CODE_SUB" & vbCrLf _
            & ", CUST_ITEM_CODE, CUST_COLOR_CODE, CUST_SIZE_CODE" & vbCrLf _
            & ", CUST_UPC, CUST_SKU" & vbCrLf
        Fill_Records("SOTORDRD", "", , ASCMAIN1.sql)

        For Each rowSOTORDRD As DataRow In dst.Tables("SOTORDRD").Select("")
            Dim MLNO As Int32 = Val(rowSOTORDRD.Item("MLNO"))
            Dim ITEM_CODE As String = rowSOTORDRD.Item("ITEM_CODE")

            Dim sqlw As String = "ITEM_CODE = '" & ITEM_CODE & "'"

            For Each COLUMN_NAME As String In New String() _
                {"ITEM_CODE_SUB", "CUST_ITEM_CODE", "CUST_COLOR_CODE", "CUST_SIZE_CODE", "CUST_UPC", "CUST_SKU"}

                If rowSOTORDRD.Item(COLUMN_NAME) & "" = "" Then
                    sqlw &= " and " & COLUMN_NAME & " is Null"
                Else
                    sqlw &= " and " & COLUMN_NAME & " = '" & rowSOTORDRD.Item(COLUMN_NAME) & "'"
                End If

                If rowSOTORDRD.Item("MLNO") = 1 And COLUMN_NAME <> "ITEM_CODE_SUB" Then
                    grdSOTORDRD.DisplayLayout.Bands(0).Columns(COLUMN_NAME).Hidden = _
                        (Val(dst.Tables("SOTORDRC").Compute("Count(ORDR_LNO)", COLUMN_NAME & " IS NOT NULL") & "") = 0)
                End If
            Next

            For Each rowSOTORDRC As DataRow In dst.Tables("SOTORDRC").Select(sqlw)
                rowSOTORDRC.Item("MLNO") = MLNO
            Next
        Next

        For Each row As DataRow In ASCDATA1.SelectDistinct(dst.Tables("SOTORDRC"), New String() {"ORDR_NO", "MLNO"}).Rows
            Dim ORDR_NO As String = row.Item("ORDR_NO")
            Dim MLNO As Int64 = Val(row.Item("MLNO"))
            Dim sqlw As String = "ORDR_NO = '" & ORDR_NO & "' and MLNO = " & CStr(MLNO)
            If Val(dst.Tables("SOTORDRC").Compute("COUNT(ORDR_LNO)", sqlw) & "") > 1 Then
                MsgBox("Redundant Style Entries on Order " & row.Item("ORDR_NO"), _
                       MsgBoxStyle.OkOnly, "Multi-Store Maintenance Does Not Support this Condition")
                disable_update = True
            End If
        Next

        ASCMAIN1.sql = "Select Count (*) from SOTORDR1 where ORDR_GROUP_NO = '" & ORDR_GROUP_NO & "'"
        Dim ords As Int32 = Val(ASCDATA1.GetDataValue)

        grdSOTORDRD.Text = "Multiple Order Maintenance; Order Group " & ORDR_GROUP_NO & "; " & ords & " Orders"
        Sort_grdColumns(grdSOTORDRD, "ITEM_CODE,COLOR_CODE")
        Setup_SOTORDRC()
    End Sub
    ...
    Private Sub cmdAddC_Click()
        If grdSOTORDRC.ActiveRow IsNot Nothing AndAlso grdSOTORDRC.ActiveRow.DataChanged Then
            grdSOTORDRC.ActiveRow.Update()
        End If

        Dim ITEM_CODE As String = Select_Item()

        If ITEM_CODE <> "" Then

            Dim MLNO_new As Int64 = Val(dst.Tables("SOTORDRC").Compute("Max (MLNO)", "") & "") + 1
            Dim ORDR_LNO_new As Int64 = Val(dst.Tables("SOTORDRC").Compute("Max (ORDR_LNO)", "") & "") + 1

            Dim MLNO_orig As Int64 = Val(grdSOTORDRD.ActiveRow.Cells("MLNO").Value & "")

            For Each rowSOTORDRC_orig As DataRow In dst.Tables("SOTORDRC") _
                    .Select("MLNO = " & CStr(MLNO_orig) & " and SUB = 1")
                Dim rowSOTORDRC As DataRow = dst.Tables("SOTORDRC").NewRow
                For i As Integer = 0 To dst.Tables("SOTORDRC").Columns.Count - 1
                    rowSOTORDRC.Item(i) = rowSOTORDRC_orig.Item(i)
                Next i
                With rowSOTORDRC
                    .Item("ORDR_LNO").Value = ORDR_LNO_new
                    .Item("MLNO").Value = MLNO_new
                    .Item("ITEM_CODE").Value = ITEM_CODE
                    .Item("ORDR_QTY").Value = rowSOTORDRC_orig.Item("ORDR_QTY_OPEN")
                    .Item("ORDR_QTY_PICK").Value = 0
                    .Item("ORDR_QTY_SHIP").Value = 0
                    .Item("ORDR_QTY_CANC").Value = 0
                    .Item("ORDR_LNO_ORIG").Value = rowSOTORDRC_orig.Item("ORDR_LNO")
                End With
                dst.Tables("SOTORDRC").Rows.Add(rowSOTORDRC_orig)
            Next

            Dim rowSOTORDRD_orig As DataRow = dst.Tables("SOTORDRD").Rows.Find(New Object() {MLNO_orig})
            Dim rowSOTORDRD As DataRow = dst.Tables("SOTORDRD").NewRow

            For i As Integer = 0 To dst.Tables("SOTORDRD").Columns.Count - 1
                rowSOTORDRD.Item(i) = rowSOTORDRD_orig.Item(i)
            Next i
            With rowSOTORDRD
                .Item("MLNO").Value = MLNO_new
                .Item("ITEM_CODE").Value = ITEM_CODE
                .Item("ORDR_QTY").Value = rowSOTORDRD_orig.Item("ORDR_QTY_OPEN")
                .Item("ORDR_QTY_PICK").Value = 0
                .Item("ORDR_QTY_SHIP").Value = 0
                .Item("ORDR_QTY_CANC").Value = 0
                .Item("ORDR_CXL").Value = 0
                .Item("ORDR_SUB").Value = 0
            End With
            dst.Tables("SOTORDRD").Rows.Add(rowSOTORDRD)

            Sort_grdColumns(grdSOTORDRD, "ITEM_CODE")
        End If
    End Sub
    ...
    Private Sub cmdSubC_Click()
        If grdSOTORDRC.ActiveRow.DataChanged Then
            grdSOTORDRC.ActiveRow.Update()
        End If

        With grdSOTORDRD.ActiveRow
            If Val(.Cells("ORDR_QTY_PICK").Value & "") = 0 _
           And Val(.Cells("ORDR_QTY_SHIP").Value & "") = 0 _
           And Val(.Cells("ORDR_QTY_CANC").Value & "") = 0 Then
                Dim ITEM_CODE As String = Select_Item()
                If ITEM_CODE <> "" Then
                    Dim MLNO As Integer
                    Dim MLNO_orig As Integer = Val(.Cells("MLNO").Value)
                    For Each rowSOTORDRC As DataRow In dst.Tables("SOTORDRC").Select("MLNO = " & .Cells("MLNO").Value & "   and SUB = -1")
                        rowSOTORDRC.Item("ITEM_CODE") = ITEM_CODE
                        If .Cells("ITEM_CODE_SUB").Value & "" = "" Then
                            rowSOTORDRC.Item("ITEM_CODE_SUB") = .Cells("ITEM_CODE").Value
                        End If
                    Next

                    Dim sqlx As String = "ITEM_CODE = '" & ITEM_CODE & "'"

                    MLNO = Val(dst.Tables("SOTORDRD").Compute("MAX(MLNO)", sqlx) & "")
                    If MLNO = 0 Then
                        Dim rowSOTORDRD_MLNO As DataRow = dst.Tables("SOTORDRD").Rows.Find(.Cells("MLNO").Value)
                        Dim rowSOTORDRD As DataRow = dst.Tables("SOTORDRD").NewRow
                        rowSOTORDRD.ItemArray = rowSOTORDRD_MLNO.ItemArray
                        rowSOTORDRD.Item("ITEM_CODE") = ITEM_CODE
                        If .Cells("ITEM_CODE_SUB").Value & "" = "" Then
                            rowSOTORDRD.Item("ITEM_CODE_SUB") = .Cells("ITEM_CODE").Value
                        End If
                        MLNO = Val(dst.Tables("SOTORDRD").Compute("MAX(MLNO)", "") & "") + 1
                        rowSOTORDRD.Item("MLNO") = MLNO
                        dst.Tables("SOTORDRD").Rows.Add(rowSOTORDRD)
                    End If
                    For Each rowSOTORDRC As DataRow In dst.Tables("SOTORDRC").Select("MLNO = " & .Cells("MLNO").Value)
                        rowSOTORDRC.Item("SUB") = "0"
                        rowSOTORDRC.Item("MLNO") = MLNO
                    Next

                    Sort_grdColumns(grdSOTORDRD, "ITEM_CODE")

                    For Each GROW As UltraWinGrid.UltraGridRow In grdSOTORDRD.Rows
                        If GROW.Cells("MLNO").Value = MLNO_orig Then
                            grdSOTORDRD.ActiveRow = GROW
                        End If
                    Next
                End If
            Else
                MsgBox("Cannot Sub Styles on Lines which have been Picked, Shipped or Cancelled")
            End If
        End With
    ...
    Sub Setup_SOTORDRC()
        Dim MLNO As Integer = Val(grdSOTORDRD.ActiveRow.Cells("MLNO").Value & "")
        For Each rowSOTORDRC As DataRow In dst.Tables("SOTORDRC").Select("MLNO = " & CStr(MLNO))
            rowSOTORDRC.Item("SUB") = "0"
        Next
        Dim dvw As DataView = dst.Tables("SOTORDRC").DefaultView
        dvw.RowFilter = "MLNO = " & CStr(MLNO)
        Sort_grdColumns(grdSOTORDRC, "CUST_STORE_NO")
        grdSOTORDRC.Text = grdSOTORDRD.ActiveRow.Cells("ITEM_CODE").Value
    End Sub

    Sub Cancel_SOTORDRD()
        grdSOTORDRD.ActiveRow.Cells("ORDR_CXL").Value = "1"
        Dim MLNO As Integer = Val(grdSOTORDRD.ActiveRow.Cells("MLNO").Value & "")
        For Each rowSOTORDRC As DataRow In dst.Tables("SOTORDRC").Select("MLNO = " & CStr(MLNO))
            rowSOTORDRC.Item("ORDR_QTY_OPEN") = 0
            Dim ORDR_QTY_CANC As Int64 = Val(rowSOTORDRC.Item("ORDR_QTY") & "") _
                                       - Val(rowSOTORDRC.Item("ORDR_QTY_PICK") & "") _
                                       - Val(rowSOTORDRC.Item("ORDR_QTY_SHIP") & "")
            rowSOTORDRC.Item("ORDR_QTY_CANC") = IIf(ORDR_QTY_CANC < 0, 0, ORDR_QTY_CANC)
        Next
    End Sub

    Sub ReGet_MOM(pass As Integer)

        Dim W As Integer
        'Pass 1 - Verify Eligability
        'Pass 2 - Load the Data.
        'Pass 3 - Set-up the grid.
        ...
        Select Case pass
            ...
            Case Is = 2
                ...
                Fill_Records("SOTORDR4", ORDR_NO)
                Fill_Records("SOTORDR5", ORDR_NO)
                Fill_Records("SOTORDRC", ORDR_NO)
                Fill_Records("SOTORDRD", ORDR_NO)

                ASCMAIN1.sql = "SELECT" & vbCrLf _
                & " SOTORDR1.CUST_STORE_NO," & vbCrLf _
                & " SOTORDR1.ORDR_CUST_PO," & vbCrLf _
                & " 0 TOTAL" & vbCrLf
                If ASCMAIN1.Running_in_VS Then Stop ' 150?
                For W = 1 To 150
                    ASCMAIN1.sql &= ", SUM(DECODE(SOTORDR2.ORDR_LNO," & W & ",SOTORDR2.ORDR_QTY,0)) QTY_" & Format$(W, "00") & vbCrLf
                Next W
                ASCMAIN1.sql &= " from SOTORDR1, SOTORDR2" & vbCrLf _
                    & " where SOTORDR1.ORDR_NO = SOTORDR2.ORDR_NO" & vbCrLf _
                    & " and ORDR_GROUP_NO = '" & ORDR_GROUP_NO & "'" & vbCrLf _
                    & " group by " & vbCrLf _
                    & " SOTORDR1.CUST_STORE_NO," & vbCrLf _
                    & " SOTORDR1.ORDR_CUST_PO" & vbCrLf
                Fill_Records("SOTORDRS", "", True, ASCMAIN1.sql)
                With grdSOTORDR2.DisplayLayout.Override
                    .AllowAddNew = UltraWinGrid.AllowAddNew.No
                    .AllowDelete = DefaultableBoolean.True
                End With
                ...
                'frmOtherInfo.Enabled = True
                With grdSOTORDR4.DisplayLayout.Override
                    .AllowAddNew = UltraWinGrid.AllowAddNew.FixedAddRowOnTop
                    .AllowDelete = DefaultableBoolean.True
                End With
                Absx1.txtFor("EDI_APPOINTMENT").ReadOnly = True
                'frmShipToOption.Enabled = False
                Set_Read_Only_for_ctl(optShipTo, True)
                Display_Totals()

            Case Is = 3
                If ASCMAIN1.Running_in_VS Then Stop ' change caption of start multi-store
                With UltraExplorerBar1.Groups("Multi-Store")
                    .Visible = True
                    '.Items("Clear Multi-Store").Visible = True 
                    '.Items("").Visible = True
                End With
                tabMain.Tabs("Multi-Store").Visible = True
                txtMSCopyToStore.Visible = True

                multi_store_is_active = True
...
#Region "grdSOTORDRD"
    ...
    Private Sub grdSOTORDRD_AfterCellUpdate(sender As Object, e As Infragistics.Win.UltraWinGrid.CellEventArgs) Handles grdSOTORDRD.AfterCellUpdate
        Select Case e.Cell.Column.Key
            Case "ORDR_CXL"
                e.Cell.Row.Update()
                If e.Cell.Value = "1" Then
                    Cancel_SOTORDRD()
                Else
                    Dim MLNO As Int64 = Val(e.Cell.Row.Cells("MLNO").Value & "")
                    For Each rowSOTORDRC As DataRow In dst.Tables("SOTORDRC").Select("MLNO = " & CStr(MLNO))
                        rowSOTORDRC.Item("ORDR_QTY_OPEN") = Val(rowSOTORDRC.Item("ORDR_QTY_OPEN") & "") _
                                                          + Val(rowSOTORDRC.Item("ORDR_QTY_CANC") & "")
                        rowSOTORDRC.Item("ORDR_QTY_CANC") = 0
                    Next
                End If
                Setup_SOTORDRC()
            ...
            Case "ORDR_SUB"
                e.Cell.Row.Update()
                If Val(e.Cell.Row.Cells("ORDR_QTY_PICK").Value & "") = 0 And _
                   Val(e.Cell.Row.Cells("ORDR_QTY_SHIP").Value & "") = 0 And _
                   Val(e.Cell.Row.Cells("ORDR_QTY_CANC").Value & "") = 0 Then
                    If e.Cell.Value = "1" Then
                        Dim ITEM_CODE As String = Select_Item()
                        ...
                        End If
                    Else
                        e.Cell.Row.Cells("ITEM_CODE").Value = e.Cell.Row.Cells("ITEM_CODE_SUB").Value & ""
                        e.Cell.Row.Cells("ITEM_CODE_SUB").Value = ""
                    End If
                    e.Cell.Row.Update()

                    Dim sqlw As String = "MLNO = " & grdSOTORDRD.ActiveRow.Cells("MLNO").Value
                    For Each rowSOTORDRC As DataRow In dst.Tables("SOTORDRC").Select(sqlw)
                        rowSOTORDRC.Item("ITEM_CODE") = grdSOTORDRD.ActiveRow.Cells("ITEM_CODE").Value
                        rowSOTORDRC.Item("ITEM_CODE_SUB") = grdSOTORDRD.ActiveRow.Cells("ITEM_CODE_SUB").Value
                    Next
                    Setup_SOTORDRC()
                End If
        End Select
    ...
    Private Sub grdSOTORDRD_AfterRowActivate(sender As Object, e As System.EventArgs) Handles grdSOTORDRD.AfterRowActivate
        Setup_SOTORDRC()
    End Sub

    Private Sub grdSOTORDRD_AfterRowUpdate(sender As Object, e As Infragistics.Win.UltraWinGrid.RowEventArgs) Handles grdSOTORDRD.AfterRowUpdate

    End Sub

    Private Sub grdSOTORDRD_BeforeRowActivate(sender As Object, e As Infragistics.Win.UltraWinGrid.RowEventArgs) Handles grdSOTORDRD.BeforeRowActivate
        If grdSOTORDRC.ActiveRow IsNot Nothing AndAlso grdSOTORDRC.ActiveRow.DataChanged Then
            grdSOTORDRC.ActiveRow.Update()
        End If
    End Sub
