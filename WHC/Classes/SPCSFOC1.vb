Public Class SPCSFOC1 ' Store Focus
    Inherits WHCRPTS1
    ...
    Private Sub InitializeReportSettings(clientData As Dictionary(Of String, Object))
        SEASON_CODE = clientData("SEASON_CODE")
        RUNAS = clientData("RUN_AS")

        SUBT = ""
        If RUNAS = "R" Then
            REGION_CODE = clientData("REGION_CODE")
        ElseIf RUNAS = "T" Then
            SELL_CODE = clientData("SELL_CODE")
        End If

        RYP0 = clientData("YYYYPP_BEGIN")
        RYP1 = clientData("YYYYPP_END")

        If inTestMode Then RecordLogEntry("Exit InitializeReportSettings")
    End Sub
    ...
    Private Sub LoadBaseReportData()
        ...
        If inTestMode Then RecordLogEntry("Exit LoadBaseReportData")
    ...
    Sub Create_XLSs()
        ...
        XLS_FILENAME_base = G.TempDirectory & ASCMAIN1.Next_Control_No(Me.Name & "_SFC") & "SFC"
    ...
    Sub Create_XLS(Optional summary_type As String = "",
        ...
        Select Case summary_type
            Case "T"
                legend = " - " & SELL_TYPE & " " & summary_code
            Case "S"
                legend = " - SDS " & summary_code
            Case "R"
                legend = " - ASD " & summary_code
            Case Else
                legend = " - Corporate"

        End Select
        reportFilename &= ";" & legend & "," & My.Computer.FileSystem.GetName(XLS_FILENAME)

        ' Show_Document(XLS_FILENAME)
        ' save document to a folder and then show on web site
    End Sub
