...
#Region "ABS Standard Routines" ' These Routines should be found in all Forms which Launch from the Menu.
    ...
    Private Sub Form_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        With dst
            ...
            With .Tables.Add("DPTFCSTV")
                ...
                .PrimaryKey = New DataColumn() {.Columns("VERSION")}
    ...
    Overrides Sub Proceed_PreReq(ByVal eItemKey As String)
        ...
        Select Case eItemKey
            ...
            Case "Update"
                ...
                Dim SQL As String = "ISNULL(ITEM_CATGY_CODE,'?') <> 'C' AND ISNULL(ITEM_CATGY_CODE,'?') <> 'N' AND ISNULL(ITEM_CATGY_CODE,'?') <> 'E' AND ISNULL(ITEM_CATGY_CODE,'?') <> 'P' AND ISNULL(ITEM_CATGY_CODE,'?') <> 'F' AND PROJ_P0 <> 0"
                ...
                For Each ITEM_CATGY_CODE As String In New String() {"E", "C", "N", "P", "F"}
    ...
    Overrides Sub Mode_Settings(ByVal tf As Boolean, Optional ByVal MODE_description As String = "")
        ...
        splDPTFCSTD.Visible = ScreenMode
        grdDPTPROJ1.Visible = Not ScreenMode
    ...
    Sub Load_Record()
        ...
        & ", NVL(DPTPROJ0.ITEM_CATGY_CODE,'I') ITEM_CATGY_CODE" _
        ...
        & " where NVL(DPTPROJ0.ITEM_CATGY_CODE,'I') in ('C','N','E','I','P','F')" _
    ...
    Sub Update_Record()
        ...
        ("(ITEM_CATGY_CODE = 'C' OR ITEM_CATGY_CODE = 'E' OR ITEM_CATGY_CODE = 'N' OR ITEM_CATGY_CODE = 'P' OR ITEM_CATGY_CODE = 'F') AND PROJ_P0 <> 0", "", DataViewRowState.CurrentRows)
...
#End Region
    ...
    Sub Setup_grdDPTFCSTD()
        ...
        If optITEM_CATGY_CODE.Value = "*" Or optITEM_CATGY_CODE.Value = "$" Then
            ...
            If optITEM_CATGY_CODE.Value = "$" Then
                sql &= " and (ITEM_CATGY_CODE = 'C' or ITEM_CATGY_CODE = 'N' or ITEM_CATGY_CODE = 'E' or ITEM_CATGY_CODE = 'P' or ITEM_CATGY_CODE = 'F')"
            End If
        Else
    ...
    Sub Format_grdDPTFCSTD()
        ...
        For M As Integer = 1 To 6
            Create_Summary(grdDPTFCSTD, "QTY_FC" & Format(M, "0"), , , "###,##0")
        Next
    ...
    Private Sub btnProrate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnProrate.Click
        ...
        For Each row As DataRow In dst.Tables("DPTFCSTD").Select(dst.Tables("DPTFCSTD").DefaultView.RowFilter)
            ...
            ' If ITEM_CODE = "BB5671XM" Then Stop
    ...
    Sub Setup_DPTPROJ1()
        Dim BRAND_CODE = Absx1.txtFor("BRAND_CODE").Text
        Dim MARKET_CODE = Absx1.txtFor("MARKET_CODE").Text

        If Absx1.txtFor("BRAND_NAME").Text <> "" And Absx1.txtFor("MARKET_DESC").Text <> "" Then
            ASCMAIN1.sql = "Select * from DPTPROJ1 " _
            & " where BRAND_CODE = '" & BRAND_CODE & "'" _
            & "   and MARKET_CODE = '" & MARKET_CODE & "'"
            Fill_Records("DPTPROJ1", "", True, ASCMAIN1.sql)
            Sort_grdColumns(grdDPTPROJ1, "OPS_YYYY".ToLower)

            grdDPTPROJ1.Text = "Projection Versions for Brand " & BRAND_CODE & ", Market " & MARKET_CODE
        Else
            grdDPTPROJ1.Text = "Projection Versions"
            dst.Tables("DPTPROJ1").Rows.Clear()
        End If

        If BRAND_CODE <> "" And MARKET_CODE <> "" Then
            cmbVersion.DropDown()
            cmbVersion.CloseUp()
        End If
    ...
    Private Sub grdDPTPROJ1_DoubleClickRow(ByVal sender As Object, ByVal e As Infragistics.Win.UltraWinGrid.DoubleClickRowEventArgs) Handles grdDPTPROJ1.DoubleClickRow
        optSEASON.Value = e.Row.Cells("SEASON").Value
        cmbOPS_YYYY.Value = e.Row.Cells("OPS_YYYY").Value

        Dim VERSION As String = e.Row.Cells("VERSION").Value
        Dim rowDPTFCSTV As DataRow = dst.Tables("DPTFCSTV").Rows.Find(VERSION)
        If rowDPTFCSTV Is Nothing Then
            MsgBox("Projections is not Available for Maintenance", MsgBoxStyle.OkOnly, "Cannot Perform Requested Action")
            Exit Sub
        End If
        cmbVersion.SelectedIndex = dst.Tables("DPTFCSTV").Rows.IndexOf(rowDPTFCSTV)
        Click_Command("Load")
    End Sub
End Class