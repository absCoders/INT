...
#Region "ABS Standard Routines" ' These Routines should be found in all Forms which Launch from the Menu.
    ...
    Private Sub Form_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        With dst
            ...
            ASCMAIN1.sql = "Select ICTCOSTR.* from ICTCOSTR where ITEM_CODE = :PARM1"
            Create_TDA(.Tables.Add, "ICTCOSTR", "**", 0, True, "V")
            ...
            ASCMAIN1.sql = "Select APTACRC1.* from APTACRC1 " & vbCrLf _
                & " where APTACRC1.ITEM_CODE = :PARM1"
        ...
        Create_Summary(grdICTIRECX, New String() {"TRAN_PV", "TRAN_MV", "EXT_COST_MATLS", "QTY_REC", "QTY_INV", "QTY_REC_NOT_INV", "AMT_REC_NOT_INV"})
        ...
        'MakeTransparent(lblRF)
        'MakeTransparent(optRF)
    ...
    Overrides Sub Mode_Settings(ByVal tf As Boolean, Optional ByVal MODE_description As String = "")
        ...
        lblRF.Visible = Not InquiryMode
        optRF.Visible = Not InquiryMode

        Set_Read_Only_for_ctl(optRF, ScreenMode And (EntryMode = "E"))

        lblRFCost.Visible = False
        If ScreenMode And (EntryMode = "E") Then
            ...
            If ITEM_COST_STATUS = "P" Then
                ' leave invisible
            Else
                lblRFCost.Visible = ScreenMode And (EntryMode = "E")
                If optRF.Value = "R" Then
                    lblRFCost.Text = "Revised Cost"
                    lblRFCost.Appearance.BackColor = System.Drawing.Color.Red
                ElseIf optRF.Value = "F" Then
                    lblRFCost.Text = "Future Cost"
                    lblRFCost.Appearance.BackColor = System.Drawing.Color.Blue
                End If
            End If
    ...
    Sub Clear_Record()
        ...
        For Each TABLE_NAME As String In New String() {"APTINVH7", "APTACRC1", "ICTCOSTM", "ICTCOSTH", "ICTCOSTR", "ICTCOSTF", "ICTITEM1", "APTINVH5", "ICTIRECX", "POTORDRX", "ICTIVAR1", "ICTIVARX"}
    ...
    Sub Load_Record()
        ...
        Sort_grdColumns(grdICTCOSTH, "OPS_YYYYPP".ToLower)
        Fill_Records("ICTCOSTR", ITEM_CODE)
        ...
        rowICTCOSTM = Add_ICTCOSTM(2, dst.Tables("ICTCOSTR").Rows.Find(New String() {ITEM_CODE}))
        rowICTCOSTM = Add_ICTCOSTM(3, dst.Tables("ICTCOSTF").Rows.Find(New String() {ITEM_CODE}))
        ...
        For Each grow As UltraWinGrid.UltraGridRow In grdICTCOSTM.Rows
            If (grow.Cells("SEQ").Value = 3 And (EntryMode = "E" And optRF.Value = "F")) _
            Or (grow.Cells("SEQ").Value = 2 And (EntryMode = "E" And optRF.Value = "R")) _
        ...
        If EntryMode = "E" Then
            ...
            Dim SEQ As Integer = IIf(optRF.Value = "R", 2, 3)

            rowICTCOSTM = dst.Tables("ICTCOSTM").Select($"SEQ = {SEQ}")(0)
    ...
    Sub Update_Record()
        ...
        Dim TABLE_NAME_ICTCOSTX As String = IIf(ITEM_COST_STATUS = "P", "ICTCOSTC", IIf(optRF.Value = "R", "ICTCOSTR", "ICTCOSTF"))
        ...
        Else
            Select Case e.SourceControl.Name
                Case "grdICTIRECX"
                    ...
                    tlb_btn.SharedProps.Visible = grd.ActiveRow IsNot Nothing AndAlso grd.ActiveRow.Band.Key = "ICTIRECX_APTINVH5"
...
#End Region
    ...
    Sub Load_ICTSTATX(OPS_YYYYPP As String, ITEM_CODE As String)
        ...
        & ", 0 WHSE_QTY_COMM" _
        & ", 0 WHSE_QTY_HOLD" _
        & " from ICTSTAT1 " & IIf(OPS_YYYYPP <> "",
                                        "where OPS_YYYYPP = '" & OPS_YYYYPP & "'",
        ...
        & ", SUM (WHSE_QTY_COMM) WHSE_QTY_COMM" _
        & ", SUM (WHSE_QTY_HOLD) WHSE_QTY_HOLD" _
        & " from ICTSTAT2 " & IIf(OPS_YYYYPP <> "",
                                        "where '" & ASCMAIN1.CYP & "' = '" & OPS_YYYYPP & "'",
                                        "where ITEM_CODE = '" & ITEM_CODE & "'") _
    ...
    Function Add_ICTCOSTM(SEQ As Integer, row As DataRow) As DataRow
        ...
        If SEQ = 2 Then SEQ_DESC = "Rev"
        If SEQ = 3 Then SEQ_DESC = "Fut"
    ...
    Private Sub grdICTCOSTM_InitializeRow(sender As Object, e As Infragistics.Win.UltraWinGrid.InitializeRowEventArgs) Handles grdICTCOSTM.InitializeRow
        If EntryMode = "E" And optRF.Value = "R" And e.Row.Cells("SEQ").Value = 2 Then
            e.Row.Appearance.BackColor = Color.LightGreen
        ElseIf EntryMode = "E" And optRF.Value = "F" And e.Row.Cells("SEQ").Value = 3 Then
            e.Row.Appearance.BackColor = Color.LightGreen
